...
#define MT6735_EVB_BUILD_PASS  //for build pass only
    ...
    int **gpuloading)
{
    ...
    if (gpufreq)
    {
#ifndef MT6735_EVB_BUILD_PASS //Jerry fix build error FIX_ME
    ...
    int *batt_temp)
{
    ...
    if (batt_current)
    {
    	*batt_current=1;//make for build pass 2014.11.24
//make for build pass 2014.11.24        *batt_current = get_sys_battery_info("/sys/devices/platform/battery/FG_Battery_CurrentConsumption");
...
//make for build pass 2014.11.24    if (batt_voltage)
 //make for build pass 2014.11.24       *batt_voltage = get_sys_battery_info("/sys/class/power_supply/battery/batt_vol");

//make for build pass 2014.11.24    if (batt_temp)
  //make for build pass 2014.11.24      *batt_temp = get_sys_battery_info("/sys/class/power_supply/battery/batt_temp");
...
//make for build pass 2014.11.24        mtk_mdm_get_md_info(&p_info, &size);
//make for build pass 2014.11.24        THRML_LOG("[mtk_thermal_get_gpu_info] mtk_mdm_get_md_info size %d\n", size);
...
//make for build pass 2014.11.24    extra_attr_values[i] = get_sys_wifi_throughput("/proc/driver/thermal/wifi_tx_thro", 3);
...
//make for build pass 2014.11.24    ret = force_get_tbat();
