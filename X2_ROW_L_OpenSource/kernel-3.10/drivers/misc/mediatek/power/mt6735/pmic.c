...
static int mtk_regulator_enable(struct regulator_dev *rdev)
{
	...
	PMICLOG("regulator_enable(name=%s id=%d en_reg=%x vol_reg=%x)\n",rdesc->name, rdesc->id
...
static int mtk_regulator_disable(struct regulator_dev *rdev)
{
	...
	PMICLOG("regulator_disable(name=%s id=%d en_reg=%x vol_reg=%x use_count=%d)\n",rdesc->name, rdesc->id
		,mreg->en_reg,mreg->vol_reg,rdev->use_count);
...
static int mtk_regulator_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	PMICLOG("regulator_get_voltage_sel(name=%s id=%d en_reg=%x vol_reg=%x reg/sel:%d voltage:%d)\n",rdesc->name, rdesc->id
		,mreg->en_reg,mreg->vol_reg,regVal,voltage);
...
static struct mtk_regulator mtk_bucks[] = {
	...
	PMIC_BUCK_GEN(VSYS22,		PMIC_QI_VSYS22_EN,	PMIC_NI_VSYS22_VOSEL, 	1400000,	2193750,6250),	
...
bool hwPowerOn(MT65XX_POWER powerId, int powerVolt, char *mode_name)
{
	...
	if (ret1!=0 || ret2!=0)
	{
		PMICLOG("hwPowerOn err:ret1:%d ret2:%d",ret1,ret2);
		return KAL_FALSE;
	}
...
void low_battery_protect_init(void)
{
	...
	PMICLOG("Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n", 
	        MT6328_AUXADC_CON5, upmu_get_reg_value(MT6328_AUXADC_CON5),
	        MT6328_AUXADC_CON6, upmu_get_reg_value(MT6328_AUXADC_CON6),
...
pmic_set_register_value(PMIC_RG_UVLO_VTHL,0);
    ...
    PMICLOG("POWER_UVLO_VOLT_LEVEL=%d, [0x%x]=0x%x\n", 
        POWER_UVLO_VOLT_LEVEL, MT6325_CHR_CON17, upmu_get_reg_value(MT6325_CHR_CON17));
}
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    //--------------------------------------------------------
        ...
        PMICLOG("[Kernel_PMIC_INIT_SETTING_V1] 2014-07-10...\n");
...
ret = pmic_config_interface(0xE,0x1,0x1,10); // [10:10]: VUSB_PG_ENB; Ricky E1 workaround
...
ret = pmic_config_interface(0x204,0x1,0x1,6); // [6:6]: RG_OSC_SEL_HW_MODE; Juinn-Ting: E1 only
...
ret = pmic_config_interface(0x278,0x1,0x1,10); // [10:10]: RG_AUXADC_26M_CK_PDN_HWEN; ZF
ret = pmic_config_interface(0x278,0x1,0x1,11); // [11:11]: RG_AUXADC_CK_CKSEL_HWEN; ZF
...
ret = pmic_config_interface(0x48C,0x30,0x7F,0); // [6:0]: VPROC_VOSEL_ON; VSRAM tracking,Fandy
...
ret = pmic_config_interface(0x67E,0x3,0x3,4); // [5:4]: VPA_DVS_TRANS_CTRL; Seven
