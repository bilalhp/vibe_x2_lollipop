...
extern kal_bool lenovo_battery_is_temp_45_to_pos_50();
#endif
/*End,Lenovo-sw chailu1 modify 2014-6-4, support 45-50 cv*/
 // ============================================================ //
 //define
 // ============================================================ //
#define STATUS_OK	0
...
static int is_po_charger_in()
{
	kal_uint32 det_count = 0;
	kal_uint32 i;

unsigned int ret = 0;

    ret = mt_set_gpio_mode(GPIO_PO_CHARGER_DET_PIN, GPIO_MODE_00);
    if(0 != ret)
    {
        printk("po mt_set_gpio_mode fail\n");
    }
    ret = mt_set_gpio_dir(GPIO_PO_CHARGER_DET_PIN, GPIO_DIR_IN);
    if(0 != ret)
    {
        printk("po mt_set_gpio_dir fail\n");
    }
    ret = mt_set_gpio_pull_enable(GPIO_PO_CHARGER_DET_PIN, GPIO_PULL_DISABLE);
    if(0 != ret)
    {
        printk("po mt_set_gpio_pull_enable fail\n");
    }

	for(i=0;i<5;i++)
	{
		msleep(1);
		if(mt_get_gpio_in(GPIO_PO_CHARGER_DET_PIN))
		{
            det_count++;
		}		    
	}

	printk("is_po_charger_in() det_count = %d \n",det_count);
    if( det_count >= 5)
    {
		return 1;
    }    
...
/*Begin,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
    ...
    {
	    battery_xlog_printk(BAT_LOG_FULL, "charging_hw_init temp in 40-50 \r\n");

	    bq24250_set_vreg(0x1e);	//4.1V
	    bq24250_set_iterm(7);//225mA   
	}
    else
    {
    #if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
      	bq24250_set_vreg(0x2b);//4.36V
    #else
    	bq24250_set_vreg(0x23);//4.2V
    #endif
    bq24250_set_iterm(2);//100mA
...
#if defined (LENOVO_BQ24250_CHG_CUR_DET) 
	bq24250_set_iinlim(0x06); //externa ilim current limit
#else
...
/*lenovo-sw weiweij modified for changing input current to 3A end*/
 ...
 /*End,Lenovo-sw chailu1 modify 2014-6-4, support 45-50 cv*/	
	...
	//bq24250_set_iterm(4);
	//lenovo_sw chailu1 add 2014-07-09 for vdpm
	bq24250_set_vin_dpm(3);
	...
	/*lenovo-sw chailu modified for safty timer to 9hr*/
	bq24250_set_tmr(2);
...
/*	else if(*data==CHARGE_CURRENT_MAX)
 ...
 if(*(kal_uint32 *)data == CHARGE_CURRENT_900_00_MA)
        register_value = 3;
#endif
...
kal_bool charging_type_detection_done(void)
{
 ...
 static kal_uint32 charging_get_charger_type(void *data)
 {
	...
	/*Begin lenovo sw-chailu1 add for po charging 2014-06-10*/
                 ...
                 if(is_po_charger_in())
                 {
				     *(CHARGER_TYPE*)(data) = PO_CHARGER;
				     battery_xlog_printk(BAT_LOG_CRTI, "step B1 : PO CHARGER!\r\n");
				 }
				 else
				 {
				     *(CHARGER_TYPE*)(data) = NONSTANDARD_CHARGER;
