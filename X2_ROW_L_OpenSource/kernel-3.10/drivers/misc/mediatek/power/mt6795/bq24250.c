...
/*Begin lenovo-sw chailu1 add for ata test 2014-07-02*/	
static ssize_t show_bq24250_ata(struct device *dev,struct device_attribute *attr, char *buf)
{
	kal_uint8 bq24250_ata_test;
	int ret = 0;
	
	ret = bq24250_read_byte(bq24250_CON0, &bq24250_ata_test);
	battery_xlog_printk(BAT_LOG_CRTI,"show_bq24250_atao:ret=%d\r\n",ret);
	if(ret<0)
        return sprintf(buf, "%u\n", 0);
	else
	    return sprintf(buf, "%u\n", 1);	
}
static ssize_t store_bq24250_ata(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(bq24250_ata, 0664, show_bq24250_ata, store_bq24250_ata); //664
...
{    
    ...
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_bq24250_ata);
