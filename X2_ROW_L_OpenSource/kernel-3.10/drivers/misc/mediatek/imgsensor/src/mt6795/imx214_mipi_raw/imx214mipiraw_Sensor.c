...
static void sensor_init(void)
{
    ...
    #if 0
    kal_uint16 tmp;
    tmp=read_cmos_sensor(0x50c3);
    tmp&=0xf8;
    tmp|=0x02;  //default 50 ohm,set 87.5 ohm
    write_cmos_sensor(0x50c3,tmp); //hs data lane impedance.
    
    tmp=read_cmos_sensor(0x50c2);
    tmp&=0xf8;
    tmp|=0x02;  //default 50 ohm,set 43.8 ohm
    write_cmos_sensor(0x50c2,tmp); //hs clock lane impedance.
    
    tmp=read_cmos_sensor(0x50c5);
    tmp&=0xf0;
    tmp|=0x0c;  //default bias 0.4v,set 0.35v
    write_cmos_sensor(0x50c5,tmp); // bias voltage.
...
static kal_uint32 get_default_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 *framerate) 
{
	...
	if(framerate==NULL)
	{
		LOG_INF("-------framerate is NULL pointer------\n");
		return ERROR_NONE;
	}
	switch (scenario_id) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_CROP_INFO:
			...
			if(wininfo==NULL)
			{
				LOG_INF("------wininfo is NULL----feature_id=[%d]\n",feature_id);
				break;
