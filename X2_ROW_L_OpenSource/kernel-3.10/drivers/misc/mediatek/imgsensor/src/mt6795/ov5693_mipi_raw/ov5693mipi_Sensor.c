...
#define SENSOR_BURST_OP 1
#if SENSOR_BURST_OP 
extern int iBurstWriteReg(u8 *pData, u32 bytes, u16 i2cId);
#define I2C_BUFFER_LEN 254 
static inline kal_uint16 OV5693MIPI_table_write_cmos_sensor(kal_uint16* para, kal_uint32 len)
{
	//at most 2 bytes address and 6 bytes data for multiple write. MTK i2c master has onl    y 8bytes fifo.
	...
	while(IDX < len)
	{
		...
		{
			iBurstWriteReg(puSendCmd , tosend, imgsensor.i2c_write_id);
			tosend = 0;
		}
	}
	return 0;
...
#define OV5693MIPI_USE_OTP 1
#if OV5693MIPI_USE_OTP
static kal_bool ov5693_otp_shading_read = KAL_TRUE;
static int otp_lenc[62];
#define BG_Ratio_Typical 0x11E
#define RG_Ratio_Typical 0x119
...
int read_otp_wb(int index, struct otp_struct *otp_ptr)
{
	...
	mdelay(5);
	address = 0x3d00;
	(*otp_ptr).module_integrator_id = read_cmos_sensor(address + 1);
	(*otp_ptr).lens_id = read_cmos_sensor(address + 2);
	(*otp_ptr).production_year = read_cmos_sensor(address + 3);
	(*otp_ptr).production_month = read_cmos_sensor(address + 4);
	(*otp_ptr).production_day = read_cmos_sensor(address + 5);
	temp = read_cmos_sensor(address + 10);
	(*otp_ptr).rg_ratio = (read_cmos_sensor(address + 6)<<2) + ((temp>>6) & 0x03);
	(*otp_ptr).bg_ratio = (read_cmos_sensor(address + 7)<<2) + ((temp>>4) & 0x03);
	(*otp_ptr).light_rg = (read_cmos_sensor(address + 8) <<2) + ((temp>>2) & 0x03);
	(*otp_ptr).light_bg = (read_cmos_sensor(address + 9)<<2) + (temp & 0x03);
	(*otp_ptr).user_data[0] = read_cmos_sensor(address + 11);
	(*otp_ptr).user_data[1] = read_cmos_sensor(address + 12);
	(*otp_ptr).user_data[2] = read_cmos_sensor(address + 13);
	(*otp_ptr).user_data[3] = read_cmos_sensor(address + 14);
	(*otp_ptr).user_data[4] = read_cmos_sensor(address + 15);
	LOG_INF("-----(*otp_ptr).rg_ratio =0x%x, (*otp_ptr).bg_ratio=0x%x\n",(*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);
	// clear otp buffer
	for (i=0;i<16;i++) {
		write_cmos_sensor(0x3d00 + i, 0x00);
	}
...
int read_otp_lenc(int index)
{
	...
	for(i=0;i<15;i++) 
	{
		otp_lenc[i]=read_cmos_sensor(address);
		LOG_INF("read ADRESS:=0x%x, PARA:=0x%x\n ",address,otp_lenc[i]);
		address++;
	}
...
{
	LOG_INF("OV5693DB_update_awb_gain R_gain =0x%x, G_gain=0x%x, B_gain=0x%x\n",R_gain, G_gain, B_gain);

	if (R_gain>0x400) 
	{
		write_cmos_sensor(0x3400, R_gain>>8);
		write_cmos_sensor(0x3401, R_gain & 0x00ff);
	}
	if (G_gain>0x400) 
	{
		write_cmos_sensor(0x3402, G_gain>>8);
		write_cmos_sensor(0x3403, G_gain & 0x00ff);
	}
	if (B_gain>0x400) 
	{
		write_cmos_sensor(0x3404, B_gain>>8);
		write_cmos_sensor(0x3405, B_gain & 0x00ff);
	}
	return 0;
}
// otp_ptr: pointer of otp_struct
int update_lenc(void)
{
	...
	for(i=0;i<62;i++) 
	{
		write_cmos_sensor(0x5800 + i, otp_lenc[i]);
		LOG_INF("OV5693OTP ADRESS:=0x%x, PARA:=0x%x\n ",0x5800 + i,otp_lenc[i]);
...
int update_otp_wb()
{
	...
	if (i>3) {
		...
		LOG_INF("[OV5693OTP]No WB OTP Data\n");
		return 1;
	}
...
int update_otp_lenc()
{
	...
	LOG_INF("ov5693_otp_shading_read =  %d", ov5693_otp_shading_read);
...
LOG_INF("E\n");
	...
	write_cmos_sensor(0x3021,0x20);  //internal regulator off
...
#else
	kal_uint16 init_para[]={
		0x0103,0x01,//);  // Software Reset
		0x3001,0x0a,//);  // FSIN output, FREX input
		0x3002,0x80,//);  // Vsync output, Href, Frex, strobe, fsin, ilpwm input
		0x3006,0x00,//);  // output value of GPIO
		0x3011,0x21,//);  //MIPI 2 lane enable
		0x3012,0x09,//);  //MIPI 10-bit
		0x3013,0x10,//);  //Drive 1x
		0x3014,0x00,//);
		0x3015,0x08,//);  //MIPI on
		0x3016,0xf0,//);
		0x3017,0xf0,//);
		...
		0x301b,0xb4,//);
		0x301d,0x02,//);
		0x3021,0x20,//);  //internal regulator off
		0x3022,0x01,//);
		0x3028,0x44,//);
		0x3098,0x03,//);  //PLL
		0x3099,0x1e,//);  //PLL
		...
		0x30a2,0x01,//);
		0x30b2,0x00,//);
		0x30b3,0x64,//);  //PLL 50 64 6c 6a->64 modify by yan 5/15
		0x30b4,0x03,//);  //PLL
		0x30b5,0x04,//);  //PLL
		0x30b6,0x01,//);  //PLL
		0x3104,0x21,//);  //sclk from dac_pll
		0x3106,0x00,//);  //sclk_sel from pll_clk
		0x3400,0x04,//);  //MWB R H
		0x3401,0x00,//);  //MWB R L
		0x3402,0x04,//);  //MWB G H
		0x3403,0x00,//);  //MWB G L
		0x3404,0x04,//);  //MWB B H
		0x3405,0x00,//);  //MWB B L
		0x3406,0x01,//);  //MWB gain enable
		0x3500,0x00,//);  //exposure HH
		0x3501,0x3d,//);  //exposure H	3d
		0x3502,0x00,//);  //exposure L
		0x3503,0x07,//);  //gain 1 frame latch, VTS manual, AGC manual, AEC manual
		0x3504,0x00,//);  //manual gain H
		0x3505,0x00,//);  //manual gain L
		0x3506,0x00,//);  //short exposure HH
		0x3507,0x02,//);  //short exposure H
		0x3508,0x00,//);  //short exposure L
		0x3509,0x10,//);  //use real ggain
		0x350a,0x00,//);  //gain H 
		0x350b,0x40,//);  //gain L
		0x3600,0xbc,//);  //(20140110)
		0x3601,0x0a,//);  //analog
		0x3602,0x38,//);
		0x3612,0x80,//);
		0x3620,0x44,//);  //0x54 (20140110)
		0x3621,0xb5,//);  //0xc7 (20140110)
		0x3622,0x0c,//);  //0x0f (20140110)
		0x3625,0x10,//);
		0x3630,0x55,//);
		0x3631,0xf4,//);
		0x3632,0x00,//);
		...
		0x37c5,0x00,//);  //sensor FREX exp HH				   
		0x37c6,0x00,//);  //sensor FREX exp H				   
		0x37c7,0x00,//);  //sensor FREX exp L				   
		0x37c9,0x00,//);  //strobe Width HH					   
		0x37ca,0x00,//);  //strobe Width H					   
		0x37cb,0x00,//);  //strobe Width L					   
		0x37de,0x00,//);  //sensor FREX PCHG Width H 		   
		0x37df,0x00,//);  //sensor FREX PCHG Width L
		...
		0x3806,0x07,//);  //Y end H							   
		0x3807,0xa3,//);  //Y end L							   
		0x3808,0x05,//);  //X output size H	2592			 //05  0a
		0x3809,0x10,//);  //X output size L						 //10  20
		0x380a,0x03,//);  //Y output size H	1944			 //03  07
		0x380b,0xcc,//);  //Y output size L						 //cc	98
		0x380c,0x0a,//);  //HTS H  2688							   
		0x380d,0x80,//);  //HTS H							   
		0x380e,0x07,//);  //HTS L  1984							 //05
		0x380f,0xc0,//);  //HTS L  //d0
		0x3810,0x00,//);  //timing ISP x win H				   
		0x3811,0x08,//);  //timing ISP x win L	  //02			   
		0x3812,0x00,//);  //timing ISP y win H				   
		0x3813,0x02,//);  //timing ISP y win L				   
		0x3814,0x31,//);  //timing x inc 			/31  11
		0x3815,0x31,//);  //timing y ing 	   //31  11
		0x3820,0x04,//);  //v fast bin on, v flip off, v bin off 		  //04	00
		0x3821,0x1f,//);  //hsync on, mirror on, hbin on 			 //1f	1e
		0x3823,0x00,//);
		0x3824,0x00,//);
		0x3825,0x00,//);
		0x3826,0x00,//);
		...
		0x3b02,0x00,//);  //strobe dummy line H	   
		0x3b03,0x00,//);  //strobe dummy line L	   
		0x3b04,0x00,//);  //strobe at next frame    
		0x3b05,0x00,//);  //strobe pulse width	   
		0x3e07,0x20,//);
		0x4000,0x08,//);
		0x4001,0x04,//);  //BLC start li
		0x4002,0x45,//);  //BLC auto enable, do 5 frames
		0x4004,0x08,//);  //8 black lines					  
		0x4005,0x18,//don't output black line, apply one channel offset to all, BLC triggered by gain change
		0x4006,0x20,//);  //ZLINE COEF							  
		0x4008,0x24,//); 							  
		0x4009,0x40,//);  //black level target	 
		0x400c,0x00,//);  //BLC man level0 H 
		0x400d,0x00,//);  //BLC man level0 L 					  
		0x4058,0x00,//); 		
		0x404e,0x37,//);  //BLC maximum black level
		0x404f,0x8f,//);  //BLC stable range 	
		0x4058,0x00,//); 	
		0x4101,0xb2,//); 			  
		0x4303,0x00,//);  //test pattern off 	  
		0x4304,0x08,//);  //test pattern option
		0x4307,0x30,//); 		 
		0x4311,0x04,//);  //Vsync width H	  
		0x4315,0x01,//);  //Vsync width L		  
		0x4511,0x05,//); 			 
		0x4512,0x01,//);
		0x4800,0x24,//);  // MIPI line sync enable non-continue mode, no ls,le
		0x4806,0x00,//); 		
		0x4816,0x52,//);  //embedded line data type	
		0x481f,0x30,//);  //max clk_prepare						  
		0x4826,0x2c,//);  //hs prepare min						  
		0x4831,0x64,//);  //UI hs prepare	 
		0x4d00,0x04,//);  //temprature sensor 
		0x4d01,0x71,//); 				
		0x4d02,0xfd,//); 				 
		0x4d03,0xf5,//); 				 
		0x4d04,0x0c,//); 				 
		0x4d05,0xcc,//);  //temperature sensor
		0x4837,0x0a,//);  //MIPI global timing  //0d 0a 09->0a modify by yan 5/15
		0x5000,0x06,//);  //BPC on, WPC on	 
		0x5001,0x01,//);  //MWB on			 
		0x5002,0x00,//);  //scal off 		 
		0x5003,0x20,//);
		0x5046,0x0a,//);  //SOF auto mode 
		0x5013,0x00,//); 		   
		0x5046,0x0a,//);  //SOF auto mode 
		0x5780,0xfc,//); //DPC
		0x5781,0x13,//);
		...
		0x5790,0x01,//);
		0x5791,0xff,//); //DPC
		0x5842,0x01,//);  //LENC BR Hscale H
		0x5843,0x2b,//);  //LENC BR Hscale L
		0x5844,0x01,//);  //LENC BR Vscale H
		0x5845,0x92,//);  //LENC BR Vscale L
		0x5846,0x01,//);  //LENC G Hscale H 
		0x5847,0x8f,//);  //LENC G Hscale L 
		0x5848,0x01,//);  //LENC G Vscale H 
		0x5849,0x0c,//);  //LENC G Vscale L 
		0x5e00,0x00,//);  //test pattern off
		0x5e10,0x0c,//);
		0x0100,0x01,//);  //wake up	 
	};
	OV5693MIPI_table_write_cmos_sensor(init_para, sizeof(init_para)/sizeof(kal_uint16));
#endif	
	LOG_INF("------[%s][%d]--init end---\n",__FUNCTION__,__LINE__);
#if OV5693MIPI_USE_OTP
	//wb otp update
	update_otp_wb();
...
static void preview_setting(void)
{
   LOG_INF("-----[%s][%d]------\n",__FUNCTION__,__LINE__);
	...
	write_cmos_sensor(0x0100,0x01);  //wake up	 
#else	
	kal_uint16 preview_para[]={
	0x0100,0x00,//);  //software sleep
	0x3098,0x03,//);  //PLL
	0x3099,0x1e,//);  //PLL
	0x309a,0x02,//);  //PLL
	0x3708,0xe6,//);  //sensor control
	0x3709,0xc7,//);  //sensor control
	0x3803,0x00,//);  //timing Y start L
	0x3806,0x07,//);  //timing Y end H
	0x3807,0xa3,//);  //timing Y end L
	0x3808,0x05,//);  //X output size H	1296
	0x3809,0x10,//);  //X output size L
	0x380a,0x03,//);  //Y output size H	972
	0x380b,0xcc,//);  //Y output size L
	0x380c, ((imgsensor_info.pre.linelength >> 8) & 0xFF),// hts = 2688
	0x380d, (imgsensor_info.pre.linelength & 0xFF),//;		  // hts
	0x380e, ((imgsensor_info.pre.framelength >> 8) & 0xFF),//);	// vts = 1984
	0x380f, (imgsensor_info.pre.framelength & 0xFF),//); 		// vts	  
	0x3810,0x00,//);  //timing ISP x win H
	0x3811,0x08,//);  //timing ISP x win L
	0x3812,0x00,//);  //timing ISP y win H
	0x3813,0x02,//);  //timing ISP y win L
	0x3814,0x31,//);  //timing X inc
	0x3815,0x31,//);  //timing Y inc
	0x3820,0x04,//);  //v fast bin on, v flip off, v bin off
	0x3821,0x1f,//);  //hsync on, mirror on, hbin on
	};
	
	OV5693MIPI_table_write_cmos_sensor(preview_para, sizeof(preview_para)/sizeof(kal_uint16));
	if (imgsensor.ihdr_en) {
...
static void capture_setting(kal_uint16 currefps)
{
   LOG_INF("E! currefps:%d\n",currefps);
	LOG_INF("-----[%s][%d]----fps=%d--\n",__FUNCTION__,__LINE__,currefps);
