...
static void msdc_mdelay(u32 time)
{
   ...
   while(t_end > arch_counter_get_cntpct()) cpu_relax();
...
static unsigned int simp_mmc_power_up(struct simp_mmc_host *host,bool on)
{
	...
	case 0:
		if (on) {
			...
			msdc_power_set_field(REG_VEMC33_VOLSEL, MASK_VEMC33_VOLSEL, VEMC33_VOLSEL_3V3);
...
static unsigned int simp_mmc_enable_clk(struct simp_mmc_host *host)
{
    ...
    mt_reg_sync_writel(0x07020202, topckgen_reg_base + MSDC_CLK_CFG_3_OFFSET);
...
static void simp_msdc_dma_stop(struct simp_msdc_host *host)
{
    ...
    if(retry == 0){
        printk("################################ Failed to stop DMA! please check it here #################################");
        //BUG();
    }
}
...
#if MTK_MMC_DUMP_DBG
			pr_err("[%s]: successfully disabled cache ops.\n", __func__);
#endif
		}
	} else {
#if MTK_MMC_DUMP_DBG
		pr_err("[%s]: cache is not enabled, no need to disable it\n", __func__);
#endif
	}
#endif
...
#if MTK_MMC_DUMP_DBG
		pr_err("init emmc for ipanic dump\n");
#endif
...
/* unsigned int i; */
	...
	int err = 0;
#if MTK_MMC_DUMP_DBG
	pr_err("1 l_start_offset = 0x%x len =0x%x\n", l_addr, len);
#endif
	if (0 != len % 512) {
		pr_err("debug: parameter error!\n");
		return ret;
...
#if MTK_MMC_DUMP_DBG
	pr_err("l_start_offset = 0x%x len = %d\n", offset, len);
#endif
	if (pmsdc_extend_host->card->card_cap == standard_capacity) {
		l_addr = offset << 9;
	} else {
...
#if MTK_MMC_DUMP_DBG
	pr_err("card_dump_func_write len<%d> addr<%lld> type<%d>\n", len, offset, dev);
#endif
	if (offset % 512) {
...
#if MTK_MMC_DUMP_DBG
	pr_err("read start sector = %llu, part size = %llu\n", (u64)lp_start_sect, (u64)lp_nr_sects);
#endif

	l_start_offset = (u64)offset + (u64)(lp_start_sect << 9);
#if MTK_MMC_DUMP_DBG
	pr_err("read start address = %llu\n", l_start_offset);
#endif
...
#if DEBUG_MMC_IOCTL
	pr_err("l_user_begin_num = %d l_dest_num = %d\n", l_user_begin_num, l_dest_num);
#endif
...
#if DEBUG_MMC_IOCTL
	pr_err("read start address=0x%llx\n",
		 PartInfo[l_dest_num].start_address - PartInfo[l_user_begin_num].start_address);
#endif
	l_start_offset =
