...
atomic_t autok_is_abort = ATOMIC_INIT(0);
...
static void autok_add_suggest_vol(unsigned int voltage);
static bool is_sug_more_than_real(int id);
...
bool is_autok_lifecycle_before = false;
int wait_sdio_autok_ready(void *data){
...
#ifdef CONFIG_SDIOAUTOK_SUPPORT    
    int is_screen_off;
    //ALPS01932171 is_autok_lifecycle_before = true; 
#endif    
    unsigned int vcore_uv = 0;
    int ret = 0;
    ...
    if((vcorefs_sdio_need_multi_autok()) && (btmod == META_BOOT))
        return 0;
    if((btmod != NORMAL_BOOT) && (btmod != META_BOOT))
        return 0;
    if (1/*(btmod!=META_BOOT)*/){
#ifdef CONFIG_SDIOAUTOK_SUPPORT    
    is_autok_lifecycle_before = true; //ALPS01932171 
#endif    
        sdio_host_debug = 0;
...
//    id = 3;
    ...
    if (id >= HOST_MAX_NUM) {
        pr_debug("[%s] id<%d> is bigger than HOST_MAX_NUM<%d>\n", __func__, id, HOST_MAX_NUM);
        return count;
    }
...
static int trigger_1_0_autok_func(void *data);
        ...
        case SS_CORNER:
            ...
            if(is_sug_more_than_real(id)){
                for (i = 0; i < HOST_MAX_NUM; i++) {
                    if (p_autok_thread_data->p_autok_progress[i].host_id == id){
                        //p_autok_thread_data->p_autok_progress[i].done = 0;
                        //task2 = kthread_run(&trigger_1_0_autok_func, (void *)&id, "autok_1_0_task");
                        trigger_1_0_autok_func(NULL);
                        break;
                    }
                }
            }
            ...
            break;
        case PARAM_COUNT:
            // [Workaround] Do reset for device
            // id can be the selection for connected device
            sscanf(buf, "%d", &id); 
#ifdef USE_WIFI_RESET       
            mtk_wcn_cmb_stub_do_reset(3);
#endif
            break;
        default:
            break;
...
static int trigger_1_0_autok_func(void *data)
{
    //ret = mtk_wcn_cmb_stub_1vautok_for_dvfs();
    //mtk_wcn_cmb_stub_func_ctrl(3,1);
    //mtk_wcn_cmb_stub_func_ctrl(3,0);
    //mtk_wcn_cmb_stub_func_ctrl(3,1);
    //mtk_wcn_cmb_stub_func_ctrl(3,0);
    mtk_wcn_cmb_stub_1vautok_for_dvfs();
    pr_debug("[AUTO_K][%s] 2. Wifi 1.0v Autok Simulation\n", __func__);
    return 0;
}
...
static void autok_add_suggest_vol(unsigned int voltage)
{
    unsigned int *temp_suggest_vol;
    int i;
    for(i=0; i<suggest_vol_count; i++){
        if(suggest_vol[i] == voltage)
            break;
    }
    if(i==suggest_vol_count){
        suggest_vol_count += 1;
        temp_suggest_vol = kzalloc(sizeof(unsigned int)*suggest_vol_count, GFP_KERNEL);
        if(suggest_vol_count > 1)
            memcpy(temp_suggest_vol, suggest_vol, sizeof(unsigned int)*(suggest_vol_count-1));
        temp_suggest_vol[suggest_vol_count-1] = voltage;
        kfree(suggest_vol);
        suggest_vol = temp_suggest_vol;
    }  
}

static bool is_sug_more_than_real(int id)
{
    unsigned int real_vol, sug_vol;
    int real_vol_count;
    int i, j;
    
    real_vol_count = p_autok_predata[id].vol_count;
    pr_debug("[AUTO_K][%s] RealCount:%d, SugCount:%d\n", __func__, real_vol_count, suggest_vol_count);
    if(suggest_vol_count > real_vol_count)
       return true;
    
    for(i=0; i<suggest_vol_count; i++){
        sug_vol = suggest_vol[i];
        for(j = 0; j<real_vol_count; j++){
            real_vol = p_autok_predata[id].vol_list[j];
            if(sug_vol == real_vol)
                break;
        }
        if(j == real_vol_count){
...
{
    int i;
    if(!is_autok_lifecycle_before)
        return 2;
    if(p_autok_predata == NULL)
        return 0;
    for(i = 0; i<p_autok_predata[id].vol_count; i++){
        if(p_autok_predata[id].vol_list[i] == voltage)
          return 1;
    }
    return 0;
}
...
int autok_abort_action(void)
{
    pr_debug("[AUTO_K][%s] AUTOK Abort\n", __func__);
    atomic_set(&autok_is_abort, 1);
    return 0;
}
