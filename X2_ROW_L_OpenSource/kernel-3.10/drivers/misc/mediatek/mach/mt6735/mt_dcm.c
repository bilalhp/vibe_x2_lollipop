...
int dcm_mcusys(ENUM_MCUSYS_DCM on)
{
        if (on == MCUSYS_DCM_OFF) {
                //MCUSYS bus DCM
                reg_write(MCUCFG_CCI_CLK_CTRL, 
                          aor(reg_read(MCUCFG_CCI_CLK_CTRL), ~(0x1<<8), 0));  
                //L2C SRAM DCM
                reg_write(MCUCFG_L2C_SRAM_CTRL, aor(reg_read(MCUCFG_L2C_SRAM_CTRL), ~(1<<0), 0));  
                //bus_fabric_dcm_ctrl
                reg_write(MCUCFG_BUS_FABRIC_DCM_CTRL, 
                          aor(reg_read(MCUCFG_BUS_FABRIC_DCM_CTRL), ~(0x0f0f), 0)); 
        }
        else {
                //MCUSYS bus DCM
                reg_write(MCUCFG_CCI_CLK_CTRL, aor(reg_read(MCUCFG_CCI_CLK_CTRL), ~(0x1<<8), (1<<8)));
                //L2C SRAM DCM
                reg_write(MCUCFG_L2C_SRAM_CTRL, aor(reg_read(MCUCFG_L2C_SRAM_CTRL), ~(1<<0), (1<<0)));
                //bus_fabric_dcm_ctrl
                reg_write(MCUCFG_BUS_FABRIC_DCM_CTRL, 
...
#define PMIC_DCM_DEFAULT (1<<21)
#define USB_DCM_DEFAULT (1<<22)
