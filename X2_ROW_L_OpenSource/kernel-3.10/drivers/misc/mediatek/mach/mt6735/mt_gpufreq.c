...
// TODO: waiting for Vcore DVFS ready
...
static unsigned int g_gpufreq_vcore_hpm = GPU_DVFS_VOLT0;   // will be changed by PTPOD
static const unsigned int g_gpufreq_vcore_lpm = GPU_DVFS_VOLT1;
static unsigned int g_last_gpu_dvs_result = 0;
...
static struct mt_gpufreq_power_table_info mt_gpufreqs_power_default[] = {
    {.gpufreq_khz = GPU_DVFS_FREQ1, .gpufreq_volt = GPU_DVFS_VOLT0, .gpufreq_power = 720},
    {.gpufreq_khz = GPU_DVFS_FREQ2, .gpufreq_volt = GPU_DVFS_VOLT0, .gpufreq_power = 448},
    {.gpufreq_khz = GPU_DVFS_FREQ2, .gpufreq_volt = GPU_DVFS_VOLT1, .gpufreq_power = 374},
};
static unsigned int mt_gpufreqs_power_num = ARRAY_SIZE(mt_gpufreqs_power_default);
...
{
    // TODO: waiting for efuse API, default use 450MHz(FY) table
    return 1;
}
 ...
 ***************************************/
static void _mt_gpufreq_power_calculation(unsigned int idx, unsigned int temp)
{
...
#define GPU_ACT_REF_VOLT	    115000	/* mV x 100 */
    ...
    p_dynamic = p_dynamic *
        ((mt_gpufreqs_power_default[idx].gpufreq_khz * 100) / ref_freq) *
        ((mt_gpufreqs_power_default[idx].gpufreq_volt * 100) / ref_volt) *
        ((mt_gpufreqs_power_default[idx].gpufreq_volt * 100) / ref_volt) /
        (100 * 100 * 100);

    p_leakage = mt_spower_get_leakage(MT_SPOWER_VCORE, (mt_gpufreqs_power_default[idx].gpufreq_volt / 100), temp);

    p_total = p_dynamic + p_leakage;

    mt_gpufreqs_power[idx].gpufreq_khz = mt_gpufreqs_power_default[idx].gpufreq_khz;
    mt_gpufreqs_power[idx].gpufreq_volt = mt_gpufreqs_power_default[idx].gpufreq_volt;    
...
static void _mt_update_gpufreqs_power_table(void)
{
...
#if 0   // TODO: waiting for thermal API
    ...
    if((temp > 0) && (temp < 125)) {
        for (i = 0; i < mt_gpufreqs_power_num; i++) {
            _mt_gpufreq_power_calculation(i, temp);
...
{
    int i = 0;
    unsigned int new_vcore_hpm = _mt_gpufreq_pmic_wrap_to_volt(pmic_get_register_value(PMIC_VCORE1_VOSEL_ON));
    ...
    for (i = 0; i < num; i++) {
        ...
        // Apply new Vcore HPM modified by PTPOD
        if (freqs[i].gpufreq_volt == GPU_DVFS_VOLT0)
        ...
        gpufreq_dbg("mt_gpufreqs[%d].gpufreq_idx = %u\n", i, mt_gpufreqs[i].gpufreq_idx);
    }
    ...
    for (i = 0; i < mt_gpufreqs_power_num; i++) {
        // modify GPU_DVFS_VOLT0 in default power table
        if (mt_gpufreqs_power_default[i].gpufreq_volt == GPU_DVFS_VOLT0) {
            mt_gpufreqs_power_default[i].gpufreq_volt = new_vcore_hpm;
            g_gpufreq_vcore_hpm = new_vcore_hpm;
        }
        // modify freq in default power table according to table type
        if (i == 0 && mt_gpufreq_dvfs_table_type == 0) {   // change freq to 600M
            mt_gpufreqs_power_default[i].gpufreq_khz = GPU_DVFS_FREQ0;
        }
    }
...
static unsigned int _mt_gpufreq_set_cur_volt(unsigned int volt_new)
{
    gpufreq_dbg("@%s: volt_new = %d \n", __func__, volt_new);

    // TODO: waiting for Vcore DVFS API
#if 0    
    if (volt_new == g_gpufreq_vcore_hpm)
        g_last_gpu_dvs_result = vcorefs_request_dvfs_opp(KR_GPU, OPP_0);
    else if (volt_new == g_gpufreq_vcore_lpm)
        g_last_gpu_dvs_result = vcorefs_request_dvfs_opp(KR_GPU, OPP_1);
    else
        g_last_gpu_dvs_result = 0xFF;   // invalid new volt!
#endif
...
{
    // TODO: waiting for Vcore DVFS API to get current Vcore   
    return 0;//vcorefs_get_curr_voltage() / 10;
}
 ...
 ************************************************************/
static int _mt_gpufreq_set(unsigned int freq_old, unsigned int freq_new, unsigned int volt_old, unsigned int volt_new)
{
    int ret = 0;

    if(freq_new > freq_old) {
        ret = _mt_gpufreq_set_cur_volt(volt_new);
...
#ifndef DISABLE_PBM_FEATURE
    {
        ...
        unsigned int cur_volt = ((ret == 0) ? volt_new : volt_old);

        for (i = 0; i < mt_gpufreqs_num; i++) {
            if (mt_gpufreqs_power[i].gpufreq_khz == freq_new &&
                ...
                gpufreq_dbg("@%s: request GPU power = %d, cur_volt = %d\n", 
                    __func__, mt_gpufreq_request_gpu_power_to_pbm, cur_volt / 100);
                break;
            }
        }
...
static void _mt_gpufreq_set_initial(void)
{
    ...
    if (i == mt_gpufreqs_num) {
        ...
        _mt_gpufreq_set(cur_freq, mt_gpufreqs[mt_gpufreqs_num - 1].gpufreq_khz,
                                    cur_volt, mt_gpufreqs[mt_gpufreqs_num - 1].gpufreq_volt);
...
void mt_gpufreq_low_batt_volume_callback(BATTERY_PERCENT_LEVEL low_battery_volume)
{
    ...
    if (low_battery_volume == BATTERY_PERCENT_LEVEL_1) {
        if (mt_gpufreq_low_batt_volume_limited_index != mt_gpufreq_low_bat_volume_limited_index_1) {
            ...
            _mt_gpufreq_low_batt_volume_protect(mt_gpufreq_low_bat_volume_limited_index_1); // Limit GPU 312Mhz
        }
    }
    //unlimit gpu
    else {
...
void mt_gpufreq_low_batt_volt_callback(LOW_BATTERY_LEVEL low_battery_level)
{
    ...
    if (low_battery_level == LOW_BATTERY_LEVEL_1) {
        ...
        if (mt_gpufreq_low_batt_volt_limited_index != mt_gpufreq_low_bat_volt_limited_index_2) {
            ...
            _mt_gpufreq_low_batt_volt_protect(mt_gpufreq_low_bat_volt_limited_index_2); // Limit GPU 312Mhz
        }
    }
    //unlimit gpu
    else {
...
unsigned int mt_gpufreq_target(unsigned int idx)
{
    ...
    target_idx = mt_gpufreqs[g_gpufreq_max_id].gpufreq_idx;
    target_OPPidx = g_gpufreq_max_id;
    gpufreq_dbg("Keep MAX frequency %dKHz for performance test!\n", target_freq);
#endif
     ...
     *******************************/
    ret = _mt_gpufreq_set(cur_freq, target_freq, cur_volt, target_volt);
    if (!ret)
...
unsigned int mt_gpufreq_voltage_enable_set(unsigned int enable)
{
    ...
    if(enable == 0) {
        // TODO: waiting for Vcore DVFS ready!
        //vcorefs_request_dvfs_opp(KR_GPU, OPP_OFF);
    }
    ...
    if (enable == 1 && state_orig == 0)
        kicker_pbm_by_gpu(true, mt_gpufreq_request_gpu_power_to_pbm, g_cur_gpu_volt / 100);      
    else if (enable == 0 && state_orig == 1)
        kicker_pbm_by_gpu(false, 0, g_cur_gpu_volt / 100);   
#endif
#endif
...
static int _mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
    ...
    if (mt_gpufreq_dvfs_table_type == 0)    // 600M
...
{   
    ...
    seq_printf(m, "g_last_gpu_dvs_result = 0x%x\n", g_last_gpu_dvs_result);
...
{    
    ...
    if (sscanf(desc, "%d %d", &fixed_freq, &fixed_volt) == 2) {
        if((fixed_freq == 0) && (fixed_volt == 0)) {
            ...
            int is_volt_available = 0;
            int is_freq_available = 0;
            ...
            // volt (mV)
            if (fixed_volt >= (_mt_gpufreq_pmic_wrap_to_volt(0x0) / 100) && 
                fixed_volt <= (_mt_gpufreq_pmic_wrap_to_volt(0x7F) / 100)) {
                mt_gpufreq_fixed_voltage = fixed_volt * 100;
                is_volt_available = 1;
            }

            if (is_freq_available) {
                if (!is_volt_available) // keep current volt
                    mt_gpufreq_fixed_voltage = _mt_gpufreq_get_cur_volt();
                mt_gpufreq_fixed_freq_volt_state = true;
                //mt_gpufreq_voltage_enable_set(1);
                mt_gpufreq_target(0);
            }
            else if (is_volt_available) {   // only fix volt
                mt_gpufreq_fixed_frequency = _mt_gpufreq_get_cur_freq();
                mt_gpufreq_fixed_freq_volt_state = true;                
                //mt_gpufreq_voltage_enable_set(1);
                mutex_lock(&mt_gpufreq_lock);
                // TODO: waiting for Vcore API (need this?) 
                //_mt_gpufreq_set_cur_volt(mt_gpufreq_fixed_voltage);
                g_cur_gpu_OPPidx = 0;
                mutex_unlock(&mt_gpufreq_lock);
            }
        }
