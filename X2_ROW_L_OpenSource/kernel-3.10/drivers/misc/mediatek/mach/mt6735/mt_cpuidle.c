...
#define MP0_DBGAPB_BASE (0xf0181000)
#define MP1_DBGAPB_BASE (0xf01C1000)
...
#define MP0_DBGAPB_BASE (0x10181000)
#define MP1_DBGAPB_BASE (0x101C1000)
...
static inline unsigned int mp0_l2rstdisable(int l2reset)
{
	unsigned int read_back = reg_read(MP0_CA7L_CACHE_CONFIG);
	int reg_val = _aor(read_back, ~L2RSTDISABLE, (!l2reset) ? L2RSTDISABLE : 0);
	reg_write(MP0_CA7L_CACHE_CONFIG, reg_val);
	
	return read_back & L2RSTDISABLE;
}

static inline unsigned int mp1_l2rstdisable(int l2reset)
{
	unsigned int read_back = reg_read(MP1_CA7L_CACHE_CONFIG);
	int reg_val = _aor(read_back, ~L2RSTDISABLE, (!l2reset) ? L2RSTDISABLE : 0);
...
// l2rstdisable imply "not l2reset", vice versa.
#define mp0_l2rstdisable_restore(f) mp0_l2rstdisable(!(f))
#define mp1_l2rstdisable_restore(f) mp1_l2rstdisable(!(f))
...
static void mt_cluster_save(int flags)
{
	...
	if (cluster_id() == 0) {
		GET_CLUSTER_DATA()->l2rstdisable = mp0_l2rstdisable(IS_DORMANT_INNER_OFF(flags));
	} 
        else {
		GET_CLUSTER_DATA()->l2rstdisable = mp1_l2rstdisable(IS_DORMANT_INNER_OFF(flags));
	}
}
...
static void mt_cluster_restore(int flags)
{
	...
	if (cluster_id() == 0) {
		mp0_l2rstdisable_restore(GET_CLUSTER_DATA()->l2rstdisable);
	} 
        else {
		mp1_l2rstdisable_restore(GET_CLUSTER_DATA()->l2rstdisable);
	}
}
...
{	
	...
	/** FIXME,
         ...
         *
         * Right now, for ROME, copy is prefered for internal debug usage.
...
int mt_cpu_dormant(unsigned long flags)
{
	...
	read_id(&cpuid, &clusterid);
        ...
        if ((core->count & 0x01ff) == 0) 
                CPU_DORMANT_INFO("dormant(%d) flags:%lx start (cluster/cpu:%d/%d) !\n", 
...
_back:
        ...
        if ((_IS_DORMANT_SET(flags, DORMANT_CPUSYS_OFF))
            || (core->count & 0x01ff) == 0) {
                CPU_DORMANT_INFO("dormant(flags:%lx) (ret:%x) (core:%d/%d) cnt:%d, rst:%d, abt:%d, brk:%d\n",
	...
	// init dormant_data
        ...
        if (1) { 
                __asm__ __volatile__(
                        ...
                        "isb \n\t"
                        "dsb \n\t"
                        : "=r" (dormant_data[0].poc.l2ctlr), "=r"(dormant_data[0].poc.l2actlr)
                        ::"memory");
                        
                CPU_DORMANT_INFO("dormant init (cluster/cpu:%d/%d), l2ctlr(%lx) !\n", 
