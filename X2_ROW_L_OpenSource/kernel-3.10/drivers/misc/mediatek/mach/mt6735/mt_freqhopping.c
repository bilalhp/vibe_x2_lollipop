...
static void __iomem *g_fhctl_dase;
static void __iomem *g_apmixed_dase;
static void __iomem *g_ddrphy_dase;
#endif
...
#if 0 //only for build pass and will be remove after home screen
...
static int __reg_base_addr_init(void)
{
    ...
    if (! fhctl_node) 
    {
        ...
        g_fhctl_dase = (void*)FHCTL_BASE;   
    }else 
    {
        g_fhctl_dase = of_iomap(fhctl_node, 0);
        if(!g_fhctl_dase) 
        {
        ...
        {
            //FH_MSG_DEBUG("FHCTL dase address:0x%lx", (unsigned long)g_fhctl_dase);     
        }
    }/* if-else */ 
    ...
    if (! fhctl_node) 
    {
        ...
        g_apmixed_dase = (void*)APMIXED_BASE;   
    }else 
    {
        g_apmixed_dase = of_iomap(apmixed_node, 0);
        if(!g_apmixed_dase) 
        {
        ...
        {
            //FH_MSG_DEBUG("APMIXED dase address:0x%lx", (unsigned long)g_apmixed_dase);                
        }
    }/* if-else */ 
    ...
    {
        FH_MSG_DEBUG(" Error, Cannot find FHCTL device tree node"); 
        g_ddrphy_dase = (void*)DDRPHY_BASE;   
    }else 
    {
        g_ddrphy_dase = of_iomap(ddrphy_node, 0);
        if(!g_ddrphy_dase) 
        {
        ...
        {
            //FH_MSG_DEBUG("DDRPHY dase address:0x%lx", (unsigned long)g_ddrphy_dase);                             
        }
    }/* if-else */ 
...
static int __fh_debug_proc_read(struct seq_file* m, void* v, fh_pll_t * pll)
{
	...
	seq_printf(m, "  =%04d==%04d==%04d==%04d==%04d==%04d==%04d==%04d==%04d=\r\n" ,
				pll[FH_ARM_PLLID].fh_status,	pll[FH_MEM_PLLID].fh_status,
	...
	seq_printf(m, "  =%04d==%04d==%04d==%04d==%04d==%04d==%04d==%04d==%04d=\r\n" ,
				pll[FH_ARM_PLLID].setting_id,pll[FH_MEM_PLLID].setting_id,
