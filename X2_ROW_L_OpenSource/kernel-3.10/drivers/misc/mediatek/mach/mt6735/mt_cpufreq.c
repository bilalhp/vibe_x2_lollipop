...
#define MAX_VSRAM_VOLT             (131000)  /* 1310mv * 100 */
...
/* PMIC WRAP ADDR */ // TODO: include other head file (pwrap_hal.h)
#ifdef CONFIG_OF
...
#define PMIC_WRAP_DVFS_ADR8     (PWRAP_BASE_ADDR + 0x128)
#define PMIC_WRAP_DVFS_WDATA8   (PWRAP_BASE_ADDR + 0x12C)
#define PMIC_WRAP_DVFS_ADR9     (PWRAP_BASE_ADDR + 0x130)
#define PMIC_WRAP_DVFS_WDATA9   (PWRAP_BASE_ADDR + 0x134)
...
/* Power throttling */  // TODO: check this!
...
/* Forward references                                    */
/*=============================================================*/
#ifdef CONFIG_CPU_DVFS_HAS_EXTBUCK  // TODO: remove extbuck API ref.
...
#ifdef __KERNEL__
// Freq Meter API
extern unsigned int mt_get_cpu_freq(void);  // TODO: remove this?
#endif
...
static struct pmic_wrap_setting pw = {
    ...
    .set[PMIC_WRAP_PHASE_SUSPEND] = {
        ...
        ._[IDX_SP_VSRAM_SHUTDOWN]   = { MT6328_PMIC_VSRAM_EN_ADDR,        _BITS_(0 : 0,  0), },
        ._[IDX_SP_VRF18_0_SHUTDOWN] = { MT6328_PMIC_RG_VRF18_0_EN_ADDR,   _BITS_(1 : 1,  0), }, 
        ._[IDX_SP_VSRAM_PWR_ON]     = { MT6328_PMIC_VSRAM_EN_ADDR,        _BITS_(0 : 0,  1), },
    ...
    .set[PMIC_WRAP_PHASE_DEEPIDLE] = {
        ...
        ._[IDX_DI_VSRAM_SLEEP]      = { MT6328_PMIC_VSRAM_VOSEL_CTRL_ADDR, _BITS_(1 : 1,  0), },
...
/* DVFS table */
/* CPU LEVEL 0, 1.5GHz segment (useless) */
...
{
#if 1
    return CPU_LEVEL_1; // TODO: fix LV1 for bringup
#else
    unsigned int lv = 0;
...
static void _mt_cpufreq_pmic_table_init(void)
{
    struct pmic_wrap_cmd pwrap_cmd_default[NR_PMIC_WRAP_CMD] = {
        ...
        { PMIC_WRAP_DVFS_ADR8, PMIC_WRAP_DVFS_WDATA8, },
        { PMIC_WRAP_DVFS_ADR9, PMIC_WRAP_DVFS_WDATA9, },
        // CMD10~15 Not used
        { 0, 0, },
        { 0, 0, },
        { 0, 0, },
...
#if 0   // TODO: should move to Vcore DVFS?
/* SDIO */
...
unsigned int mt_get_cur_volt_vcore_pdn(void)
{
    ...
    rdata = pmic_get_register_value(PMIC_VCORE1_VOSEL_ON);
...
static int _mt_cpufreq_set_cur_volt_locked(struct mt_cpu_dvfs *p, unsigned int volt)  // volt: mv * 100
{
...
#endif
                VOLT_TO_PMIC_VAL(mv + (cpu_dvfs_is(p, MT_CPU_DVFS_LITTLE) ? 0 : NORMAL_DIFF_VSRAM_VPROC))
                );
    ...
    if (   cpu_dvfs_is(p, MT_CPU_DVFS_BIG)
        ...
        mt_cpufreq_set_pmic_cmd(PMIC_WRAP_PHASE_SUSPEND,
                ...
                VOLT_TO_PMIC_VAL(volt + NORMAL_DIFF_VSRAM_VPROC)
                );
#endif
...
static unsigned int _mt_cpufreq_get_cur_phy_freq(struct mt_cpu_dvfs *p)
{
    ...
    con2 = cpufreq_read(ARMPLL_CON2);
    con1 = (con1 & _BITMASK_(31 : 21)) | (con2 & _BITMASK_(20 : 0));
 ...
 * CPU freq scaling
 *
 * above 1040MHz: use freq hopping
 * below 1040MHz: set ARMPLL and CLKDIV
 * if cross 1040MHz, migrate to 1040MHz first.
 * 
 */
static void _mt_cpufreq_set_cur_freq(struct mt_cpu_dvfs *p, unsigned int cur_khz, unsigned int target_khz)
{
...
#define IS_CLKDIV_USED(clkdiv)  (((clkdiv < 8) || ((clkdiv % 8) == 0)) ? 0 : 1)
    ...
    if (!is_fhctl_used) {    
        ...
        switch (target_khz) {
            case CPU_DVFS_FREQ4:
                dds = _mt_cpufreq_cpu_dds_calc(CPU_DVFS_FREQ4);
                sel = 8;    // 4/4
                break;
            case CPU_DVFS_FREQ5:
                dds = _mt_cpufreq_cpu_dds_calc(CPU_DVFS_FREQ5);
                sel = 8;    // 4/4
                break;
            case CPU_DVFS_FREQ6:
                dds = _mt_cpufreq_cpu_dds_calc(CPU_DVFS_FREQ6 * 2);
                sel = 10;    // 2/4
                break;
            case CPU_DVFS_FREQ7:
                dds = _mt_cpufreq_cpu_dds_calc(CPU_DVFS_FREQ7 * 2);
                sel = 10;    // 2/4
                break;
            case CPU_DVFS_FREQ8:
                dds = _mt_cpufreq_cpu_dds_calc(CPU_DVFS_FREQ8 * 4);
                sel = 11;    // 1/4
                break;                
            default:
                BUG();
        ...
        // adjust Vproc since MAINPLL is 1092 MHz (~= CPU_DVFS_FREQ3)
...
static unsigned int _mt_cpufreq_get_cur_vsram(struct mt_cpu_dvfs *p)
{
    ...
    rdata = pmic_get_register_value(PMIC_VSRAM_EN);

    if (rdata) { /* enabled i.e. not 0mv */
        int ret = 0;
        do {
            rdata = pmic_get_register_value(PMIC_VSRAM_VOSEL_ON);
        } while ((rdata == 0 || rdata > 0x7F) && retry_cnt--);
...
static int _mt_cpufreq_set_locked(struct mt_cpu_dvfs *p, unsigned int cur_khz, unsigned int target_khz, struct cpufreq_policy *policy)
{
...

    {
        ...
        mt_cpufreq_set_pmic_cmd(PMIC_WRAP_PHASE_DEEPIDLE,
                ...
                IDX_DI_VPROC_CA7_NORMAL,
#endif
                VOLT_TO_PMIC_VAL(volt + (cpu_dvfs_is(p, MT_CPU_DVFS_LITTLE) ? 0 : NORMAL_DIFF_VSRAM_VPROC))
                );
...
#elif defined(CONFIG_CPU_DVFS_PERFORMANCE_TEST)
        new_opp_idx = 0;    // fix at max freq
#else
...
void mt_cpufreq_thermal_protect(unsigned int limited_power)
{
...
#ifdef CONFIG_CPU_FREQ
    {
        ...
        cpufreq_dbg("found = %d, limited_power_idx = %d, limited_max_freq = %d, limited_max_ncpu = %d\n", 
            found, p->limited_power_idx, p->limited_max_freq, p->limited_max_ncpu);

        cpufreq_unlock(flags);                                   /* <- unlock */

        // TODO: waiting for HPS
...
#if 0   // TODO: remove this! Be used in hotplug governor
/* for ramp down */
    ...
    for_each_cpu_dvfs(i, p) {
        ...
        switch (level) {
            case BATTERY_OC_LEVEL_1:
                ...
                else if (num_possible_cpus() == 4 && p->cpu_level <= CPU_LEVEL_1)
                    // Battery OC trigger CPU Limit to under 1.2G
                    p->pwr_thro_mode |= PWR_THRO_MODE_BAT_OC_1170MHZ;
                else if (num_possible_cpus() == 4 && p->cpu_level == CPU_LEVEL_2)
...
#define DORMANT_MODE_VOLT   85000
...
#if 0  // Move to PMIC initial setting
        /******************************
         * VSRAM auto-tracking setting
         ******************************/
        // TODO: check HW auto-tracking is work or not!
        // turn on soft change for VSRAM        
        pmic_config_interface(0x04B0, 0x8888, 0xFFFF, 0x0);
        // turn on soft change for VDVFS1
        pmic_config_interface(0x04B8, 0x8888, 0xFFFF, 0x0);
        pmic_config_interface(0x0498, 0x13, 0x33, 0x0);
        // offset = 0x0; delta = 0x10 (diff = 100mV)
        pmic_config_interface(0x0424, 0x0010, 0x7F7F, 0x0);
        // (normal mode) HB = 1.35V; LB = 0.93125V
        pmic_config_interface(0x0426, 
                                        (VOLT_TO_PMIC_VAL(MAX_VSRAM_VOLT) << 16) | VOLT_TO_PMIC_VAL(MIN_VSRAM_VOLT), 
                                        0x7F7F, 
                                        0x0
                                        );
        // VSRAM_VOSEL_SLEEP_LB = 0.6V
        pmic_config_interface(0x0428, 0x0, 0x7F, 0x0);
        // set vosel as HW mode vosel for VSRAM & DVFS1
        pmic_config_interface(0x0482, 0xA, 0xF, 0x0);
        pmic_config_interface(0x04AA, 0xA, 0xF, 0x0);
        // turn on tracking in normal and sleep mode
        pmic_config_interface(0x0422, 0x7, 0xF, 0x0);
        // Vsram default = 1V
        pmic_config_interface(0x04B4, 0x40, 0x7F, 0x0);
        // VSRAM_VOSEL_SLEEP = 0
        pmic_config_interface(0x048E, 0x0, 0x7F, 0x0);
        pmic_config_interface(0x0204, 0x0173, 0x377, 0x0);
#endif
        ...
        if (cpu_dvfs_is_extbuck_valid()) {
            ...
            pmic_set_register_value(PMIC_VSRAM_VOSEL_CTRL, 0x1);
            pmic_set_register_value(PMIC_VSRAM_VOSEL_RSV, VOLT_TO_PMIC_VAL(MIN_VSRAM_VOLT));
        }
#endif
        ...
        if (_mt_cpufreq_sync_opp_tbl_idx(p) >= 0) /* sync p->idx_opp_tbl first before _mt_cpufreq_restore_default_volt() */
...
void dvfs_disable_by_ptpod(int id)
{
    ...
    case PTP_CTRL_LTE:
        vlte_backup = pmic_get_register_value(PMIC_VLTE_VOSEL);
        pmic_set_register_value(PMIC_VLTE_VOSEL, VOLT_TO_PMIC_VAL(105000));//VBOOT = 1.05
        break;
...
void dvfs_enable_by_ptpod(int id)
{
    ...
    case PTP_CTRL_LTE:
        pmic_set_register_value(PMIC_VLTE_VOSEL, vlte_backup);
        break;
