...
void mt_spower_ut(void)
{
	int v, t, p, i;
        ...
        for (i=0; i<MT_SPOWER_MAX; i++) {
				...
				switch (i) {
					case  MT_SPOWER_CPU:
						printk("[SPOWER] - This is MT_SPOWER_CPU\n");
						break;
					case  MT_SPOWER_VCORE:
						printk("[SPOWER] - This is MT_SPOWER_VCORE\n");
						break;
					case  MT_SPOWER_LTE:
						printk("[SPOWER] - This is MT_SPOWER_LTE\n");
						break;
					default:
						break;
				}
				...
				switch (i) {
					case  MT_SPOWER_CPU:
						printk("[SPOWER] - MT_SPOWER_CPU Done\n");
						break;
					case  MT_SPOWER_VCORE:
						printk("[SPOWER] - MT_SPOWER_VCORE Done\n");
						break;
					case  MT_SPOWER_LTE:
						printk("[SPOWER] - MT_SPOWER_LTE Done\n");
						break;
					default:
						break;
				}
...
#define DEVINFO_GPU_BIT (16)
	...
	if (tab_validate(&sptab[MT_SPOWER_CA7]))
		return 0;
	devinfo = (int)get_devinfo_with_index(DEVINFO_IDX0);// ptp_read(0xF020617C); /* M_HW_RES6 */

	cpu = (devinfo >> DEVINFO_MP1_BIT) & 0x0ff;
	vcore = (devinfo >> DEVINFO_MP0_BIT) & 0x0ff;
	lte = (devinfo >> DEVINFO_GPU_BIT) & 0x0ff;
	...
	printk("[SPOWER] - cpu/vcore/lte => %d/%d/%d\n", cpu, vcore, lte);
	printk("[SPOWER] - MT_SPOWER_CPU\n");
	mt_spower_make_table(&sptab[MT_SPOWER_CPU], &cpu_spower_raw, cpu, 1150, 30);
	printk("[SPOWER] - MT_SPOWER_VCORE\n");
	mt_spower_make_table(&sptab[MT_SPOWER_VCORE], &vcore_spower_raw, vcore, 1150, 30);
	printk("[SPOWER] - MT_SPOWER_LTE\n");
	mt_spower_make_table(&sptab[MT_SPOWER_LTE], &lte_spower_raw, lte, 1150, 30);
	//mt_spower_make_table(&sptab[MT_SPOWER_VCORE], &vcore_spower_raw, vcore, 1150, 30);

	printk("[SPOWER] - Start SPOWER UT!\n");
	mt_spower_ut();
	printk("[SPOWER] - End SPOWER UT!\n");
	return 0;
}
