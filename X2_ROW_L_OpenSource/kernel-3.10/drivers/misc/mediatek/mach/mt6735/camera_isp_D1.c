...
static MINT32 ISP_RTBC_DEQUE(MINT32 dma,ISP_DEQUE_BUF_INFO_STRUCT*    pdeque_buf)
{
    ...
    if (0==pdeque_buf->count){
        ...
        for ( i=0;i<=pstRTBuf->ring_buf[rt_dma].total_count-1;i++ ) {
            LOG_DBG("[rtbc][DEQUE]Buf List:%d/%d/0x%lx/0x%x/0x%x/%d/  ", \
                i, \
    ...
    if ( pdeque_buf->count ) {
        ...
        for (i=0;i<pdeque_buf->count;i++) {
            ...
            //
            if(IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
                LOG_DBG("[rtbc][DEQUE]:index(%d)/PA(0x%x)/memID(%d)/size(0x%x)/VA(0x%lx)", \
                    index+i, \
                            ...
                            if ( 0 != rt_buf_ctrl.ex_data_ptr ) {
                                ...
                                if (i == ISP_RT_BUF_SIZE) {
                                    ...
                                    for (x=0;x<ISP_RT_BUF_SIZE;x++) {
                                        LOG_DBG("[rtbc]dma(%d),idx(%d) PA(0x%x) VA(0x%lx)",rt_dma,x,pstRTBuf->ring_buf[rt_dma].data[x].base_pAddr,pstRTBuf->ring_buf[rt_dma].data[x].base_vAddr);
                                    }
                ...
                if ( (reg_val & 0x01) || (reg_val2 & 0x01) || (camsv_reg_cal[0] & 0x01) || (camsv_reg_cal[1] & 0x01)) {
                    if(MTRUE == _bFlag) {
                        ...
                        //for (i=0;i<deque_buf.count;i++) {
                            ...
                            if(IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
                                LOG_DBG("[rtbc][DEQUE](%d):d(%d)/id(0x%x)/bs(0x%x)/va(0x%x)/pa(0x%x)/t(%d.%d)/img(%d,%d,%d,%d,%d,%d,%d,%d,%d)/m(0x%x)/fc(%d)/rrz(%d,%d,%d,%d,%d,%d),dmao(%d,%d,%d,%d),lm#(0x%x)", \
                                    iBuf+i,\
                                    rt_dma, \
