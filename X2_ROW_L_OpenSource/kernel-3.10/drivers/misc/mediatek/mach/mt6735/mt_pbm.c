...
void dump_kicker_info(void)
{
	...
	pbm_crit("[***] Switch (MD1: %d, MD2: %d, GPU: %d, Flash: %d, CPU_volt: %d, GPU_volt: %d, CPU_num: %d)\n",
			hpfmgr->switch_md1,
			hpfmgr->switch_md2,
			hpfmgr->switch_gpu,
			hpfmgr->switch_flash,
	...
	pbm_crit("[***] Resource (DLPT: %ld, Leakage: %ld, MD: %ld, CPU: %ld, GPU: %ld, Flash: %ld)\n", 
			hpfmgr->loading_dlpt,
...
{	
	unsigned int _dlpt=0, leakage=0, md=0, dlpt=0, cpu=0, gpu=0, flash=0;
	unsigned int tocpu=0, togpu=0;
	int multiple=0;
	...
	pbm_crit("[%s] DLPT: %d, Leakage: %d, MD: %d, CPU: %d, GPU: %d, Flash: %d (Multiple: %d)\n", __func__, 
				dlpt, leakage, md, tocpu, togpu, flash,  multiple);
}
