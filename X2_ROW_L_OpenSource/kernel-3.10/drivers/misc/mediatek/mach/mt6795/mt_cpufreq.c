...
#define NORMAL_DIFF_VRSAM_VPROC 100 // 20
...
unsigned int bk_up_max_freq = -1;
unsigned int bk_up_max_cores = -1;

static void _set_no_limited(struct mt_cpu_dvfs *p)
{
	struct mt_cpu_dvfs *p_little;
	FUNC_ENTER(FUNC_LV_HELP);
	...
	if (!cpu_dvfs_is(p, MT_CPU_DVFS_LITTLE) && (bk_up_max_freq != -1)) {
		p_little = id_to_cpu_dvfs(_get_cpu_dvfs_id(0));
		if (cpu_dvfs_is_availiable(p_little)) {
			p->thermal_protect_limited_power = p_little->thermal_protect_limited_power;
		}
		
		p->limited_max_freq = bk_up_max_freq;
		p->limited_max_ncpu = bk_up_max_cores;
		//printk("Turn on Big cluster & re-init- (big_core:%d, big_freq:%d, limit_power = %d)\n", p->limited_max_ncpu, p->limited_max_freq, p->thermal_protect_limited_power);
...
int mt_cpufreq_thermal_protect(unsigned int limited_power)
{
		...
		if (chip_ver == CHIP_SW_VER_01) {
			...
			if (i < cpu_tlp_power_tbl[tlp].nr_power_table) {
				...
				//if (cpu_dvfs_is_availiable((&cpu_dvfs[MT_CPU_DVFS_BIG]))) {
					...
					bk_up_max_freq = cpu_dvfs[MT_CPU_DVFS_BIG].limited_max_freq;
					bk_up_max_cores = cpu_dvfs[MT_CPU_DVFS_BIG].limited_max_ncpu;
					//printk("backup core & freq => (%d, %d)\n", cpu_dvfs[MT_CPU_DVFS_BIG].limited_max_ncpu, cpu_dvfs[MT_CPU_DVFS_BIG].limited_max_freq);
				//}
...
#if 1 // TODO: FIXME, apply limit
	{
		...
		for_each_cpu_dvfs(i, p) {
			...
			//printk("set limited cpu cores - (%d, %d)\n", cpu_dvfs[MT_CPU_DVFS_LITTLE].limited_max_ncpu, cpu_dvfs[MT_CPU_DVFS_BIG].limited_max_ncpu);
			//printk("limited cpu cores freq => (%d, %d)\n", cpu_dvfs[MT_CPU_DVFS_LITTLE].limited_max_freq, cpu_dvfs[MT_CPU_DVFS_BIG].limited_max_freq);
#endif
