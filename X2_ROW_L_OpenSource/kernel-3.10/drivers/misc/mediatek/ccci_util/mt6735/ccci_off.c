...
static void internal_md2_power_down(void){
	...
	ret = md_power_on(SYS_MD2); //double confirm
	...
	printk("[ccci-off]1.power on MD2G\n");
	// power on MD2G
	...
	printk("[ccci-off]2.power off MD2G\n");
	// power off MD2G
	...
	md_power_off(/*0*/1, 0); // no need to poll, as MD SW didn't run and enter sleep mode, polling will not get result
	// double confirm
