...
void pmic_voltage_read(unsigned int nAdjust)
{
    ...
    unsigned int OldVmem = 0;

    printk("[PMIC]pmic_voltage_read : \r\n");

    ret_val = pmic_read_interface(MT6328_VCORE1_CON12, &OldVcore1, 0x7F, 0);
    ret_val = pmic_read_interface(MT6328_SLDO_ANA_CON0, &OldVmem, 0x7F, 0);

    printk("[Vcore] MT6328_VCORE1_CON12=0x%x,\r\n[Vmem] MT6328_SLDO_ANA_CON0=0x%x \r\n", OldVcore1, OldVmem);
}
...
void pmic_Vcore_adjust(int nAdjust)
{
    switch (nAdjust) {
        case 0:	// HV 1280MHz 
        	pmic_config_interface(MT6328_VCORE1_CON12, 0x6B, 0x7F, 0);	// 1.265V
        	break;
        case 1: // HV 938MHz
        	pmic_config_interface(MT6328_VCORE1_CON12, 0x59, 0x7F, 0);	// 1.155V
        	break;
        case 2:	// NV 1280MHz 
        	pmic_config_interface(MT6328_VCORE1_CON12, 0x58, 0x7F, 0);	// 1.15V
        	break;
        case 3: // NV 938MHz
        	pmic_config_interface(MT6328_VCORE1_CON12, 0x48, 0x7F, 0);	// 1.05V
        	break;
        case 4:	// LV 1280MHz 
        	pmic_config_interface(MT6328_VCORE1_CON12, 0x46, 0x7F, 0);	// 1.035V
        	break;
        case 5: // LV 938MHz
        	pmic_config_interface(MT6328_VCORE1_CON12, 0x38, 0x7F, 0);	// 0.945V
        	break;
        default:
...
int dram_do_dfs_by_fh(unsigned int freq)
{
    ...
    {     
        case 900000:
          ...
          break;          
        case 938000:
          target_dds = 0xe5010;   ///< 938Mbps
          break;          
        case 1280000:
          target_dds = 0x138800;  ///< 1280Mbps
          break;          
        case 1333000:
