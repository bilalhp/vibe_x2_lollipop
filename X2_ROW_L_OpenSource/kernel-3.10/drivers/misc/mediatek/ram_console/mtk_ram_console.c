...
unsigned int ram_console_size(void)
{
	...
	sz_max = (unsigned int)CONFIG_MTK_RAM_CONSOLE_SIZE ;
#elif defined(CONFIG_MTK_RAM_CONSOLE_USING_DRAM)
	sz_max = (unsigned int)CONFIG_MTK_RAM_CONSOLE_DRAM_SIZE ;
#endif

	if (sz_max > ram_console_buffer->sz_console)
...
//		if(num_online_cpus() > 1){
	...
	/* save log to emmc */
	buff_size = ram_console_size() + sizeof(struct ram_console_buffer);


	printk(KERN_ERR"ram_console: 0x%p, 0x%x\n", ram_console_buffer, buff_size);
