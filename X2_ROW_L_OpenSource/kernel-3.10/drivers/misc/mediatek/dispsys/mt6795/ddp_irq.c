...
irqreturn_t disp_irq_handler(int irq, void *dev_id)
{
	...
	{                                                                                                                                                                                       
		...
		/* handle frame done first, incase MMP confuse when they come together */
		if (reg_val & (1 << 2)) {                                                                          
		...
		if (reg_val & (1 << 3)) {                                                                          
			DDPERR("IRQ: RDMA%d EOF abnormal! cnt=%d\n", index,                                                
		...
		if (reg_val & (1 << 4)) {                            
			unsigned int dsi_line_cnt = DISP_REG_GET(DDP_REG_BASE_DSI0+0x16C);

			MMProfileLogEx(ddp_mmp_get_events()->rdma_underflow, MMProfileFlagPulse,cnt_rdma_underflow, dsi_line_cnt);
			MMProfileLogEx(ddp_mmp_get_events()->SCREEN_UPDATE[index], MMProfileFlagPulse, reg_val, dsi_line_cnt);                                                            
			
			DDPERR("IRQ: RDMA%d underflow! cnt=%d dsi0_cur(%d,%d)\n", index, cnt_rdma_underflow[index]++,
					DISP_REG_GET(DDP_REG_BASE_DSI0+0x168), dsi_line_cnt);
	...
	{                                                                                                                                                                                       
		...
		for (mutexID = 0; mutexID < 5; mutexID++) {                                                        
			if (reg_val & (0x1 << mutexID)) {                                                              
				...
				MMProfileLogEx(ddp_mmp_get_events()->MUTEX_IRQ[mutexID], MMProfileFlagPulse, 
					reg_val, DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(mutexID)));    
			}
		}
		for (mutexID = 0; mutexID < 5; mutexID++) {                                                        
