...
#ifdef CONFIG_LENOVO_BACKLIGHT_DAYNIGHT
...
static volatile int g_pwm_max_backlight[2] = { 1158, 1158 };  //2662*0.87 //1331*0.87
...
static int disp_pwm_config_init(DISP_MODULE_ENUM module, disp_ddp_path_config *pConfig, void *cmdq)
{
    ...
    DISP_REG_MASK(cmdq, reg_base + DISP_PWM_CON_1_OFF, 1331, 0xfff); /* 1024 levels */
...
*/
    int real_level;
    int level_255 = level_1024/4;
    if(level_255 >=0 && level_255 <=10)//(0,0),(10,10) 
	real_level = (unsigned long)level_255;

    else if(level_255 <=128 &&  level_255>10)//(10,10),(128,75)
	real_level = ((unsigned long)level_255*65 + 530)/118;

    else if(level_255 <=205 && level_255>128)//(128,75),(205,155)
	real_level = ((unsigned long)level_255* 80 - 4465)/77;

    else if(level_255 <=255 && level_255>205) //(205,155),(255,255) 
	real_level = ((unsigned long)level_255*2) - 255;

    PWM_MSG("disp_pwm_level_remap,level_1024 = %d, level_255 = %d, real_level = %d", level_1024, level_255, real_level);
	
    level_1024 = real_level * 4;

#ifdef CONFIG_LENOVO_BACKLIGHT_DAYNIGHT
    if(day_night != 1)  
    		level_1024 = level_1024*1158/1023; // normal mode
    else
		level_1024 = level_1024*1331/1023; //ultra light mode
#else
		level_1024 = level_1024*1158/1023; // normal mode
#endif
    return level_1024;
...
int disp_pwm_set_backlight_cmdq(disp_pwm_id_t id, int level_1024, void *cmdq)
{
	...
	if (old_pwm != level_1024) {
	 ...
	 //if((level_1024 > 1158)&&(day_night != 1))//if not ultra light,then max pwm is 1158
	 //	level_1024 = 1158; //max pwm duty 
         if((level_1024 > 0)&&(level_1024 < 14) )
                level_1024 = 14;
