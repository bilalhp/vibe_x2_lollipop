...
static int md_cd_dump_info(struct ccci_modem *md, MODEM_DUMP_FLAG flag, void *buff, int length)
{
    ...
    if(flag & DUMP_FLAG_DUMP_QUEUE_0) {
		...
		printk("[CCCI%d-DUMP]dump txq0 packet history, ptr=%d, tr_done=%x, tx_xmit=%x\n",md->index+1, 
				md_ctrl->tx_history_ptr[0], (unsigned int)md_ctrl->txq[0].tr_done->gpd_addr, (unsigned int)md_ctrl->txq[0].tx_xmit->gpd_addr);
		ccci_mem_dump(md->index, md_ctrl->tx_history[0], sizeof(md_ctrl->tx_history[0]));

		printk("[CCCI%d-DUMP]dump rxq0 packet history, ptr=%d, tr_done=%x\n",md->index+1, 
				md_ctrl->rx_history_ptr[0], (unsigned int)md_ctrl->rxq[0].tr_done->gpd_addr);
		ccci_mem_dump(md->index,md_ctrl->rx_history[0], sizeof(md_ctrl->rx_history[0]));
