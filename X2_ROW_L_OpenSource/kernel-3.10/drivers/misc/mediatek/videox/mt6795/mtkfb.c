...
/* lenovo-sw yexm1 add lcd effect function 20140501 start */
...
static int mtkfb_set_lcm_feature_mode(lenovo_disp_feature_state_t *states)
{
	...
	if (down_interruptible(&sem_flipping)) {
		printk("[FB Driver] can't get semaphore in mtkfb_set_lcm_feature_mode\n");
		return -ERESTARTSYS;
	}
	if (down_interruptible(&sem_early_suspend)) {
		printk("[FB Driver] can't get semaphore in mtkfb_set_lcm_feature_mode\n");
...
#endif
/* lenovo-sw yexm1 add lcd effect function 20140501 end */

/* lenovo-sw wangyq add for daynight 20140614 start */
#ifdef CONFIG_LENOVO_BACKLIGHT_DAYNIGHT
#define GPIO_DAYNIGHT_116 (GPIO116 | 0x80000000)
int day_night = 0;
static void set_backlight_daynight(unsigned int mode)
{
       printk(KERN_ERR "%s  mode is %d\n",__func__,mode);
	day_night = mode;
	mt_set_gpio_dir(GPIO_DAYNIGHT_116, 1);
	if(mode == 1)
		mt_set_gpio_out(GPIO_DAYNIGHT_116, 1);
	else if(mode == 0)
		mt_set_gpio_out(GPIO_DAYNIGHT_116, 0);
	else
		printk("%s  mode is %d\n",__func__,mode);
}
#endif
/* lenovo-sw wangyq add for daynight 20140614 end */
...
/* lenovo-sw wangyq add for daynight 20140614 start */
#ifdef CONFIG_LENOVO_BACKLIGHT_DAYNIGHT
	case MTKFB_SET_BACKLIGHT_DAYNIGHT:
		{
	    		unsigned int mode;
			...
			if (copy_from_user(&mode, (void __user *)arg, sizeof(mode))) {
        			...
        			set_backlight_daynight(mode);
            			printk("[MTKFB EM]CONFIG_LENOVO_BACKLIGHT_DAYNIGHT succeed mode is %d\n", mode);
        		}
        		return (r);
	}
#endif
