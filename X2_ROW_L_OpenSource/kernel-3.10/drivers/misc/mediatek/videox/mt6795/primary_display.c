...
#ifdef CONFIG_LENOVO_LCM_INFO_SHOW
...
int primary_display_set_mode(int sess_mode)
{
	pgc->session_mode = sess_mode;
	return 0;
}
int primary_display_get_mode()
{
...
static unsigned long long idlemgr_vsync_cnt = (0ULL);
static unsigned long long idlemgr_last_kick = (0ULL);
#define IDLEMGR_MAX_LEVEL (2)
static unsigned long long idlemgr_idle_level_thres[IDLEMGR_MAX_LEVEL+1] = 
			{0/*not idle*/, 2, 20};
static int idlemgr_cur_level = 0;

#if 0
static atomic_t sodi_disable_cnt = ATOMIC_INIT(0);
int primary_display_idle_can_enter()
{
	return !atomic_read(sodi_disable_cnt);
}
void primary_display_enable_sodi()
{
	int tmp = atomic_dec(sodi_disable_cnt);
	if(tmp == 0)
		MMProfileLogEx(ddp_mmp_get_events()->prim_sodi_stat, MMProfileFlagStart, 0, 0);
	else
		MMProfileLogEx(ddp_mmp_get_events()->prim_sodi_stat, MMProfileFlagPulse, tmp, 1);
}
void primary_display_disable_sodi()
{
	int tmp = atomic_inc(sodi_disable_cnt);
	if(tmp == 1)
		MMProfileLogEx(ddp_mmp_get_events()->prim_sodi_stat, MMProfileFlagEnd, 0, 0);
	else
		MMProfileLogEx(ddp_mmp_get_events()->prim_sodi_stat, MMProfileFlagPulse, tmp, 0);
}
void primary_display_init_sodi_stat(int enable)
{
	if(enable) {
		atomic_set(sodi_disable_cnt, 0);
		MMProfileLogEx(ddp_mmp_get_events()->prim_sodi_stat, MMProfileFlagStart, 0, 0);
	}
	if(disable) {
		atomic_set(sodi_disable_cnt, 1);
	}
}
#else
void primary_display_enable_sodi()
...
static void primary_display_idlemgr_enter_idle(int level, int need_lock)
{

	if(level == 1)	{
		/* enable sodi */
		if(primary_display_is_video_mode())
			primary_display_enable_sodi();
	}

	if(level == 2)	{
		/* switch to decouple mode */
		session_mode_before_enter_idle = primary_display_get_mode();

		if(primary_display_is_video_mode() &&
			pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE &&
			disp_helper_get_option(DISP_HELPER_OPTION_IDLEMGR_SWTCH_DECOUPLE))
		{
			/* disable SODI first, because level 1 has enabled SODI */
			primary_display_disable_sodi();
...
static void primary_display_idlemgr_leave_idle(int level, int need_lock)
{
	if(level == 1)	{
		/* disable sodi */
		if(primary_display_is_video_mode())
...
static void primary_display_idlemgr_kick(char *source, int need_lock)
{
	...
	MMProfileLogEx(ddp_mmp_get_events()->idlemgr, MMProfileFlagPulse, 1, idlemgr_vsync_cnt);

	idlemgr_last_kick = idlemgr_vsync_cnt;
	
	for(level=idlemgr_cur_level; level>0; level--) {
	
		primary_display_idlemgr_leave_idle(level, 0);
		MMProfileLogEx(ddp_mmp_get_events()->idlemgr, MMProfileFlagEnd, level, 0);
		if(level>1)
			MMProfileLogEx(ddp_mmp_get_events()->idlemgr, MMProfileFlagStart, level-1, 0);
		DISPMSG("idlemgr leave idle level %d\n", level);
	}
...
static int _primary_path_idlemgr_monitor_thread(void *data)
{
	...
	while(1)
	{   
		//wait_event_interruptible(idlemgr_wait_queue, !idlemgr_cur_level);
		...
		for(level=idlemgr_cur_level+1; level<=IDLEMGR_MAX_LEVEL; level++) {
			if(idlemgr_vsync_cnt - idlemgr_last_kick > idlemgr_idle_level_thres[level]) {

				if(level > 1)
					MMProfileLogEx(ddp_mmp_get_events()->idlemgr, MMProfileFlagEnd, level-1, 0);
				MMProfileLogEx(ddp_mmp_get_events()->idlemgr, MMProfileFlagStart, level, idlemgr_vsync_cnt);
				
				idlemgr_cur_level = level;
				primary_display_idlemgr_enter_idle(level, 0);

				DISPMSG("idlemgr enter idle level %d\n", level);
			} else {
				break;
			}
		}
...
static int _DC_switch_to_DL_fast(void)
{
	...
	//cmdqRecDumpCommand(pgc->cmdq_handle_config);
	_cmdq_set_config_handle_dirty();
	/* use blocking flush for SODI issue (we have to disable sodi UNTILL switch done) */
	/* If this cause UX issue, remember to deal with SODI here */
...
void primary_display_sodi_rule_init(void)
{
	...
	{
		primary_display_init_sodi_stat(0);
		return;
	}
	
	primary_display_init_sodi_stat(1);
	//return;
	if(primary_display_is_video_mode())
	{
...
static int lcm_info_read(struct seq_file *m, void *data)
{
    LCM_PARAMS *lcm_param = disp_lcm_get_params(pgc->plcm);
    LCM_DRIVER *lcm_drv = pgc->plcm->drv;
	
    seq_printf(m,"name=%s; width=%d; height=%d; mode=%s; lane=%d;\n",
	lcm_drv->name,
	lcm_param->width,
	lcm_param->height,
	(lcm_param->dsi.mode==0)?"CMD":"VDO",
...
static int lcm_info_proc_open(struct inode *inode, struct file *file) 
{ 
    return single_open(file, lcm_info_read, inode->i_private); 
}
static const struct file_operations lcm_info_fops = {
    .open = lcm_info_proc_open,
    .read = seq_read,
    .llseek = seq_lseek, 
...
/* lenovo-sw s1 add lcd effect function 20140314 start*/
	...
	disp_feature_state[MTKFB_DISPIF_PRIMARY_LCD].gamma_mode= 0;//set move mode as default
...
/* lenovo-sw yexm1 add lcd effect function 20140314 end*/
	...
	do{
		...
		lcm_info = proc_create("lcm_info",0444,NULL,&lcm_info_fops);
		if(lcm_info==NULL){
	...
	{   
        ...
        if(pgc->need_trigger_dcMirror_out == 0){
			...
			if(pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE) {
				...
				if(session_info)
					dprec_submit(&session_info->event_trigger, 0xffffffff, 0);
...
DISP_STATUS DISP_SetCabcMode(unsigned int mode)
{
	...
	else
	{
		...
		{	//cmdq
			...
			//DISPCHECK("primary set cmd, handle=0x%x\n", cmdq_handle_cmd);
			if(ret!=0)
			{
			...
			if(primary_display_is_video_mode())
			{
				...
				pgc->plcm->drv->set_cabcmode(cmdq_handle_cmd,mode);
...
End:
	...
	if(_should_wait_path_idle())
	    dpmgr_wait_event_timeout(pgc->ovl2mem_path_handle, DISP_PATH_EVENT_FRAME_DONE, HZ*1);//for mt6595
	if (!pgc->plcm->drv->set_gammamode) {
		ret = DISP_STATUS_NOT_IMPLEMENTED;
		goto End;
	}
	pgc->plcm->drv->set_cabcmode(mode);
	...
	else
	{
		...
		{	//cmdq
			...
			if(primary_display_is_video_mode())
			{
				...
				pgc->plcm->drv->set_inversemode(cmdq_handle_cmd,mode);
				_cmdq_flush_config_handle_mira(cmdq_handle_cmd, 1);		
				DISPCHECK("[Display]_set_cmd_by_cmdq ret=%d\n",ret);
...
End:
	...
	pgc->plcm->drv->set_inversemode(mode);
	...
	else
	{
		...
		{	//cmdq
			...
			if(primary_display_is_video_mode())
			{
				...
				pgc->plcm->drv->set_iemode(cmdq_handle_cmd,mode);
				_cmdq_flush_config_handle_mira(cmdq_handle_cmd, 1);		
				DISPCHECK("[Display]_set_cmd_by_cmdq ret=%d\n",ret);
	...
	else
	{
		...
		{	//cmdq
			...
			if(primary_display_is_video_mode())
			{
				...
				pgc->plcm->drv->set_gammamode(cmdq_handle_cmd,mode);
				_cmdq_flush_config_handle_mira(cmdq_handle_cmd, 1);		
				DISPCHECK("[Display]_set_cmd_by_cmdq ret=%d\n",ret);
			...
			else
			{
				...
				_cmdq_insert_wait_frame_done_token_mira(cmdq_handle_cmd);
				cmdqRecClearEventToken(cmdq_handle_cmd, CMDQ_SYNC_TOKEN_CABC_EOF);
				if(!strcmp(pgc->plcm->drv->name, "otm1902a_fhd_dsi_cmd_tianma_lcm_drv")) //wangyq13 add for tianma 20140830
					cmdqRecWait(cmdq_handle_cmd, CMDQ_EVENT_MDP_DSI0_TE_SOF);
				//disp_lcm_set_cmd(pgc->plcm, (void*) cmdq_handle_cmd,mode);
				if (!pgc->plcm->drv->set_gammamode) {
					ret = DISP_STATUS_NOT_IMPLEMENTED;
					DISPCHECK("[Display]set_gammamode NULL\n");
...
End:
	...
	pgc->plcm->drv->set_gammamode(mode);
End:
	return ret;
#endif
}
