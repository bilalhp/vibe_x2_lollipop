...
static int _disp_primary_path_idle_detect_thread(void* data)
{
	...
	while(1)
	{
		...
		if(((sched_clock()- last_primary_trigger_time)/1000)>500*1000)
		{
			...
			DISPMSG("[ddp_idle_off]idle for 0.5s, switch to low power mode! \n");            
...
#ifdef OVL_CASCADE_SUPPORT_INTERNAL
...
//	dpmgr_path_init(pgc->dpmgr_handle, CMDQ_DISABLE);
	...
	if ( _is_decouple_mode(pgc->session_mode))
	{
		pgc->output_type = DISP_OUTPUT_DECOUPLE;
	}
	else
	{
		pgc->output_type = DISP_OUTPUT_DIRECT_LINK;
	}
done:
...
int primary_display_merge_session_cmd(disp_session_config *config)
{
	...
	unsigned int output_type = config->output_type;

	if (DISP_OUTPUT_UNKNOWN == output_type)
	{
		output_type = DISP_OUTPUT_DECOUPLE;
	}
	pgc->output_type = output_type;
	...
	if (isAEEEnabled)
	{
		session_input->config_layer_num = primary_display_get_option("ASSERT_LAYER");
	}
	else
	{
...
int primary_display_trigger(int blocking, void *callback, unsigned int userdata)
{
    ...
    else if (pgc->output_type == DISP_OUTPUT_DECOUPLE)
    {
        uint32_t writing_mva = 0;
    ...
    else if (pgc->output_type == DISP_OUTPUT_MEMORY)
    {
        uint32_t writing_mva = 0;
...
int primary_display_config_output(disp_mem_output_config* output)
{
	...
	else if (_is_decouple_mode(pgc->session_mode) && !_is_mirror_mode(pgc->session_mode))
	{
		/*decouple multipss mode */

		cmdq_handle = pgc->cmdq_handle_ovl1to2_config;
		if(output->security) {
			//set this config handle as secure
			cmdqRecSetSecure(cmdq_handle, 1);
		}
		pconfig = dpmgr_path_get_last_config(pgc->ovl2mem_path_handle);
		pconfig->wdma_config.dstAddress 		= output->addr;
		pconfig->wdma_config.srcHeight			= output->h;
