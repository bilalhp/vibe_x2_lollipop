...
int _ioctl_trigger_session(unsigned long arg)
{
	...
	DISPPR_FENCE("T+/%s%d/type:%d\n", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id), config.output_type);
...
#if 0
	DISPMSG("_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%lx, paddr=0x%lx, src fmt=%s, dst fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u, aen=%u\n",
						dst->layer,
						dst->layer_en,
						dst->buff_idx,
						(unsigned long)(dst->vaddr),
