...
static struct kbase_va_region *kbase_mem_from_umm(kbase_context *kctx, int fd, u64 *va_pages, u64 *flags)
{
    ...
    {   
        printk(KERN_EMERG "MALI enter kbase_mem_from_umm but dma_buf_get NULL!!! fd= %d\n", fd);
        printk(KERN_EMERG "MALI dma_buf=0x%p\n", dma_buf);
		goto no_buf;
    }
...
{
	kbase_va_region * reg;
    printk(KERN_EMERG "MALI enter kbase_mem_import here!! type = %d\n",type);
	KBASE_DEBUG_ASSERT(kctx);
	KBASE_DEBUG_ASSERT(gpu_va);
	KBASE_DEBUG_ASSERT(va_pages);
	KBASE_DEBUG_ASSERT(flags);
	...
	if (!is_compat_task())
		*flags |= BASE_MEM_SAME_VA;
#endif
    printk(KERN_EMERG "MALI enter kbase_mem_import ready to switch! and handle = %d\n", handle);
	switch (type) {
        printk(KERN_EMERG "MALI kbase_mem_import switch!!!!!!\n");
#ifdef CONFIG_UMP
        printk(KERN_EMERG "MALI kbase_mem_import switch  enter CONFIG_UMP here!!!!!!!!!\n");
	case BASE_MEM_IMPORT_TYPE_UMP:
        printk(KERN_EMERG "MALI kbase_mem_import switch  enter CONFIG_UMP and case here!!!!!!!!!\n");
		reg = kbase_mem_from_ump(kctx, (ump_secure_id)handle, va_pages, flags);
		break;
#endif /* CONFIG_UMP */
#ifdef CONFIG_DMA_SHARED_BUFFER
	case BASE_MEM_IMPORT_TYPE_UMM:
        printk(KERN_EMERG "MALI kbase_mem_import switch  enter CONFIG_DMA_SHARED_BUFFER here!!!!!!!!!\n");
		reg = kbase_mem_from_umm(kctx, handle, va_pages, flags);
		break;
#endif /* CONFIG_DMA_SHARED_BUFFER */
	default:
	    printk(KERN_EMERG "MALI kbase_mem_import switch, oh shit, reg is NULL here!!!!!!!!!\n");
		reg = NULL;
		break;
	}
	...
	if (!reg)
		goto no_reg;
    printk(KERN_EMERG "MALI kbase_mem_import switch, survive!!!!!!!!!!\n");
	kbase_gpu_vm_lock(kctx);
...
no_cookie:
	...
	kfree(reg);
no_reg:
    printk(KERN_EMERG "MALI kbase_mem_import no reg here!!!!!!!\n");
	*gpu_va = 0;
	*va_pages = 0;
	*flags = 0;
	return -ENOMEM;
