	...
	case KBASE_FUNC_MEM_IMPORT:
		{
            ...
            printk(KERN_EMERG "MALI ENTER KBASE_FUNC_MEM_IMPORT!!! CAUGHT ON SPOT!!!!TYPE:%d\n", mem_import->type);
			...
			case BASE_MEM_IMPORT_TYPE_UMP:
                ...
                printk(KERN_EMERG "case: BASE_MEM_IMPORT_TYPE_UMP, handle=%d\n", handle);
				break;
			case BASE_MEM_IMPORT_TYPE_UMM:
				get_user(handle, phandle);
                printk(KERN_EMERG "case: BASE_MEM_IMPORT_TYPE_UMM, handle=%d\n", handle);
				break;
			default:
				goto bad_type;
				break;
...
bad_type:
				ukh->ret = MALI_ERROR_FUNCTION_FAILED;
                printk(KERN_EMERG "kbase_mem_import failed here\n");
			}
			break;
		}
...
{
    printk(KERN_EMERG "GPU probe() HERE!!!!!!!!\n");
	struct kbase_device *kbdev;
	...
	int attribute_count;
     printk(KERN_EMERG "GPU enter CONFIG_OF******\n");
#ifdef CONFIG_MALI_PLATFORM_FAKE
	config = kbase_get_platform_config();
    if(config == NULL)
        printk(KERN_EMERG "GPU test config if NULL!!******\n");
	attribute_count = kbasep_get_config_attribute_count(config->attributes);
    printk(KERN_EMERG "GPU attribute count is %d!! \n", attribute_count);

	err = platform_device_add_data(pdev, config->attributes,
			attribute_count * sizeof(config->attributes[0]));
    printk(KERN_EMERG "GPU err is %d!! \n", err);
	if (err)
		return err;
#endif /* CONFIG_MALI_PLATFORM_FAKE */
#endif /* CONFIG_OF */
    printk(KERN_EMERG "ready to kbase_alloc!!!!!!!!!!\n");
	kbdev = kbase_device_alloc();
	if (!kbdev) {
		dev_err(&pdev->dev, "Can't allocate device\n");
        printk(KERN_EMERG "GPU Can't allocate device!!!!!!!!\n");
		err = -ENOMEM;
		goto out;
	}
    printk(KERN_EMERG "GPU Can't allocate device end here!!!!!!!!\n");
#ifdef CONFIG_MALI_NO_MALI
	...
	kbdev->dev = &pdev->dev;
    printk(KERN_EMERG "GPU assign platform_data here!!!!!!!!\n");

	platform_data = (kbase_attribute *) kbdev->dev->platform_data;

    printk(KERN_EMERG "GPU assign platform_data end here!!!!!!!!\n");
	if (NULL == platform_data) {
		dev_err(kbdev->dev, "Platform data not specified\n");
        printk(KERN_EMERG "GPU Platform data not specified!!!!!!!!\n");
		err = -ENOENT;
		goto out_free_dev;
	}
	...
	if (MALI_TRUE != kbasep_validate_configuration_attributes(kbdev, platform_data)) {
        ...
        printk(KERN_EMERG "GPU Configuration attributes failed to validate!!!!!!!!\n");
		err = -EINVAL;
		goto out_free_dev;
	}
	...
	for (i = 0; i < 3; i++) {
        ...
        printk(KERN_EMERG "before get resource!!!!!!!!!!!!!!!!\n");
		...
		irqtag = i;
#endif /* CONFIG_OF */
        printk(KERN_EMERG "end OF_CONFIG here %d!!!!!!!!!!!!!!!!!\n", i);
		kbdev->irqs[irqtag].irq = irq_res->start;
		kbdev->irqs[irqtag].flags = (irq_res->flags & IRQF_TRIGGER_MASK);
	}

    printk(KERN_EMERG "before platform_get_resource!!!!!!!!!!!!\n");
    ...
    printk(KERN_EMERG "end platform_get_resource!!!!!!!!!!!!!\n");        
...
out:
    printk(KERN_EMERG "GPU probe END!!!!!!!!\n");
	return err;
}
...
static int __init kbase_driver_init(void)
{
    ...
    printk(KERN_EMERG "GPU init HERE!!!!!!!!\n");
