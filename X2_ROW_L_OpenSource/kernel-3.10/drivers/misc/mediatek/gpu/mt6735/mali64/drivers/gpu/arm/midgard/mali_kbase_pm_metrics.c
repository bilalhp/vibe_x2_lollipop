...
void kbasep_gpu_utilization_procfs_init(void)
{    
...
void kbasep_gpu_utilization_procfs_term(void)
{
    if (!mali_pentry)
        return;
...
#else
#define kbasep_gpu_utilization_procfs_init() do{}while(0)
#define kbasep_gpu_utilization_procfs_term() do{}while(0)
...
{
	kbdev->gpu_util_dentry = debugfs_create_file("utilization", \
					S_IRUGO, \
