...
/*lenovo-sw fuxm1 2015-04-08 add for powerup VMCH power domain for earphone begain*/
#include <mach/mt_pm_ldo.h>
/*lenovo-sw fuxm1 2015-04-08 add for powerup VMCH power domain for earphone end*/
...
{
     ACCDET_DEBUG("[accdet] chris: current gpio129 is %d\n",mt_get_gpio_out(GPIO_FSA8049_PIN));
...
//Disable ACCDET
   ...
   //KE under fastly plug in and plug out
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		#ifdef ACCDET_PIN_SWAP
			//pmic_pwrap_write(0x0400, pmic_pwrap_read(0x0400)|(1<<14)); 
      /*lenovo-sw fuxm1 2014-07-12 modify for reduce non-standard headset detect time begain*/
		    msleep(2);//msleep(800);
		    accdet_FSA8049_enable();  //enable GPIOxxx for PIN swap 
		    ACCDET_DEBUG("[Accdet] FSA8049 enable!\n");
		    msleep(2);//msleep(250); //PIN swap need ms 
      /*lenovo-sw fuxm1 2014-07-12 modify for reduce non-standard headset detect time end*/
		#endif
...
#define PIN_ADC_CHANNEL 5
    ...
    ACCDET_DEBUG("[Accdet]accdet interrupt happen:[%s]current AB = %d,ACCDET_STATE_RG = 0x%0x\n", 
	...
	switch(accdet_status)
    {
        case PLUG_OUT:
            ...
            if(current_status == 0)
            {
				...
				pmic_pwrap_write(ACCDET_PWM_WIDTH, cust_headset_settings->pwm_width);
				 ...
				 ACCDET_DEBUG("[Accdet]before adc read, pin_adc_value = %d mv!\n", pin_adc_value);
			       ...
			       msleep(50);				 
				 current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0)>>6); //A=bit1; B=bit0
				 ACCDET_DEBUG("[Accdet] chris, current_status = %d\n", current_status);	
			     /*lenovo-sw fuxm1 2014-07-12 modify for reduce non-standard headset detect time end*/
				 
				 if (current_status == 0 && show_icon_delay != 0)
				 {
				 /*lenovo-sw fuxm1 2014-07-12 add for try 3 times to stable the pin_adc_value begain */
				   int retrys =3;
				    while (--retrys != 0)
				    {
				       msleep(50);
				...
				/*lenovo-sw fuxm1 2014-07-12 add for try to stable the pin_adc_value begain */
				
			       /*lenovo-sw fuxm1 2015-02-13 modify pin_adc_value to 340 is abnormal but for currently compatiability begain*/
					//if (200 > pin_adc_value && pin_adc_value> 100) //100mv   ilegal headset
					if (240 > pin_adc_value && pin_adc_value> 68) //80mv   ilegal headset
			       /*lenovo-sw fuxm1 2015-02-13 modify pin_adc_value for compatiability end*/
					{
						mutex_lock(&accdet_eint_irq_sync_mutex);
			...
			else if(current_status == 1)
            {
			   ...
			   #ifdef ACCDET_PIN_RECOGNIZATION
/*lenovo-sw fuxm1 2014-09-02 modify dutycycle too 100% for headphone insert noise begain*/
				//pmic_pwrap_write(ACCDET_PWM_WIDTH, REGISTER_VALUE(cust_headset_settings->pwm_width));
                                //pmic_pwrap_write(ACCDET_PWM_THRESH, REGISTER_VALUE(cust_headset_settings->pwm_thresh));
                 pmic_pwrap_write(ACCDET_PWM_WIDTH, cust_headset_settings->pwm_width); //REGISTER_VALUE(cust_headset_settings->pwm_width));
                pmic_pwrap_write(ACCDET_PWM_THRESH, cust_headset_settings->pwm_thresh); //REGISTER_VALUE(cust_headset_settings->pwm_thresh));
/*lenovo-sw fuxm1 2014-09-02 modify dutycycle too 100% for headphone insert noise end*/                
			   #endif
				//#ifdef ACCDET_LOW_POWER
...
static void accdet_work_callback(struct work_struct *work)
{
    ...
    check_cable_type();
/*lenovo-sw fuxm1 add for check illegal earphone type*/
#ifdef ACCDET_PIN_RECOGNIZATION
       if (1 == cable_pin_recognition) 
       {
            cable_pin_recognition = 0;
		accdet_FSA8049_disable();  //enable GPIO129 for PIN swap 
		cable_type = HEADSET_NO_MIC;
		accdet_status = PLUG_OUT;
		ACCDET_DEBUG("[Accdet] TS3A26746E enable!\n");
       }
	   
//check cable_pin_recognition status
      //sleep(500);
      /* check_cable_type();
       if (1 == cable_pin_recognition) 
       {
		...
		cable_type = HEADSET_NO_MIC;
		accdet_status = PLUG_OUT;
		ACCDET_DEBUG("[Accdet] TS3A26746E enable 2nd times!\n");
       }*/
       else //if(0 == cable_pin_recognition);
	 {
		ACCDET_DEBUG("[Accdet] inset to TS3A26746E is china standrd!\n");          
	 } 
#endif    
/*lenovo-sw fuxm1 add end*/
...
{ 
    ...
    pmic_pwrap_write(ACCDET_DEBOUNCE0, cust_headset_settings->debounce0);
 /*lenovo-sw fuxm1 2014-07-12 modify for reduce non-standard headset detect time begain*/
   //pmic_pwrap_write(ACCDET_DEBOUNCE1, 0xFFFF); //chris modify
    pmic_pwrap_write(ACCDET_DEBOUNCE1, 0x3FFF);
/*lenovo-sw fuxm1 2014-07-12 modify for reduce non-standard headset detect time end*/
...
/*lenovo-sw fuxm1 2015-04-08 add for powerup VMCH power domain for earphone begain*/
        hwPowerOn(MT6331_POWER_LDO_VMCH,VOL_3300,"VMCH");
/*lenovo-sw fuxm1 2015-04-08 add for powerup VMCH power domain for earphone end*/	
	//------------------------------------------------------------------
