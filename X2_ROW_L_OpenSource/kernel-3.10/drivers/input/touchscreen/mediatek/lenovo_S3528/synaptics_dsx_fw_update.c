...
#if TPD_DEBUG_MORE
#define __dbg(X...)	 printk(KERN_INFO "[S3528][FWU]"X)
#else
#define __dbg(X...)
#endif

#if TPD_DEBUG_CORE
#define __dbg_core(X...)	 printk(KERN_INFO "[S3528][FWU]"X)
#else
#define __dbg_core(X...)
#endif

#ifdef FWU_AUTO_UPDATE_CTRL_USERSPACE
	#define STARTUP_FW_UPDATE_DELAY_MS 5000 /* addon 5s wait fs mounted, std: only 4s needed*/
#else
	#define STARTUP_FW_UPDATE_DELAY_MS 1000 /* 1s */
#endif
...
extern void rmi4_tpd_hw_reset(void);
extern void rmi4_tpd_reprobe(struct synaptics_rmi4_data *rmi4_data);
extern void rmi4_tpd_reinit(struct synaptics_rmi4_data *rmi4_data);
extern int rmi4_tpd_get_status(struct synaptics_rmi4_data *rmi4_data);
static void fwu_successful_callback(void);
static int fwu_is_crc_error(void);
...
static struct bin_attribute dev_attr_data = {
	.attr = {
		.name = "data",
		.mode = (S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP)/*(S_IRUGO | S_IWUGO)*/,  /*CTS test, no other permission, mediatek modify*/
	},
	.size = 0,
...
static struct device_attribute attrs[] = {
	__ATTR(doreflash, (S_IWUSR | S_IWGRP)/*S_IWUGO*/,
			synaptics_rmi4_show_error,
			fwu_sysfs_do_reflash_store),
	__ATTR(writeconfig, (S_IWUSR | S_IWGRP)/*S_IWUGO*/,
			synaptics_rmi4_show_error,
			fwu_sysfs_write_config_store),
	__ATTR(readconfig, (S_IWUSR | S_IWGRP)/*S_IWUGO*/,
			synaptics_rmi4_show_error,
			fwu_sysfs_read_config_store),
	__ATTR(configarea, (S_IWUSR | S_IWGRP)/*S_IWUGO*/,
			synaptics_rmi4_show_error,
			fwu_sysfs_config_area_store),
	__ATTR(imagename, (S_IWUSR | S_IWGRP)/*S_IWUGO*/,
			synaptics_rmi4_show_error,
			fwu_sysfs_image_name_store),
	__ATTR(imagesize, (S_IWUSR | S_IWGRP)/*S_IWUGO*/,
			synaptics_rmi4_show_error,
			fwu_sysfs_image_size_store),
	__ATTR(blocksize, (S_IRUSR | S_IRGRP)/*S_IRUGO*/,
			fwu_sysfs_block_size_show,
			synaptics_rmi4_store_error),
	__ATTR(fwblockcount, (S_IRUSR | S_IRGRP)/*S_IRUGO*/,
			fwu_sysfs_firmware_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(configblockcount, (S_IRUSR | S_IRGRP)/*S_IRUGO*/,
			fwu_sysfs_configuration_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(permconfigblockcount, (S_IRUSR | S_IRGRP)/*S_IRUGO*/,
			fwu_sysfs_perm_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(blconfigblockcount, (S_IRUSR | S_IRGRP)/*S_IRUGO*/,
			fwu_sysfs_bl_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(dispconfigblockcount, (S_IRUSR | S_IRGRP)/*S_IRUGO*/,
			fwu_sysfs_disp_config_block_count_show,
			synaptics_rmi4_store_error),
};

static struct synaptics_rmi4_fwu_handle *fwu;
static unsigned char FAMILY_QUERY_ID;
unsigned char syna_fwu_upgrade_progress = 0;

static unsigned int FW_ID_B_UPDAT;
static unsigned int curr_lastest_fwid = 1;
const static void *cfwu_data = NULL;
unsigned char tpd_rmi4_s3203_det;
...
{
	int retval;
	enum flash_area flash_area = NO_DATA;//NONE;
...
static unsigned int tpd_read_fw_version(void)
{
	int i, retval;
	unsigned int config_id_no = 0;
	unsigned char config_id[4];	
	retval = fwu->fn_ptr->read(fwu->rmi4_data,fwu->f34_fd.ctrl_base_addr,config_id,sizeof(config_id));//0X00AF
	if (retval < 0) {
		__dbg("Failed to read config (code %d).\n", retval);
		return retval;
	}
	__dbg("Device config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
		config_id[0], config_id[1], config_id[2], config_id[3]);
	
	for (i = 0; i < sizeof(config_id); i++)
	{
		config_id_no += ((int)config_id[i] << ((sizeof(config_id) - i - 1) * 8));
//		__dbg("the [%d] is 0x%08x ...\n", i, config_id_no);
	}
	if (fwu_is_crc_error())
		config_id_no = 0;
...
static unsigned int tpd_read_custom_family_query(void)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,addrss,&config_fam,sizeof(config_fam));//0X0029(0x002a)
	if (retval < 0) {
		__dbg("Failed to read family_query (code %d).\n", retval);
		return retval;
	}
	__dbg(" the address is 0x%08x,family_query is 0x%02X\n",addrss,config_fam);
...
int fwu_get_lastest_fwid(void)
{
#if AUTO_UPDATE
	return curr_lastest_fwid;
#else
	return 0;/*auto update not enable*/
#endif
}
...
{
	int st = 0;
	st = rmi4_tpd_get_status(fwu->rmi4_data);
	st = st & 0xF;	//crc error
	/*1 error, 0 ok*/
	return (st == 4 || st == 5);
}

static int fwu_need_force_upgrade(void)
{
	if ((FW_ID_B_UPDAT == 0x0) || fwu_is_crc_error())
		return 1;
	return 0;
}

int fwu_get_upgrade_progress(void)
{
#if AUTO_UPDATE
	return syna_fwu_upgrade_progress;
#else
	return FWU_PROGRESS_NOT_UPDATE;/*have not update*/
#endif
}

static int fwu_is_userspace_ctrl(void)
{
#if defined(FWU_AUTO_UPDATE_CTRL_USERSPACE)
	int found = 1,retry;
	struct file *fp;

	for(retry = 0; retry < 3; retry++) {
		fp = filp_open(PATH_USERSPACE_CTRL, O_RDONLY, 0664);
		if (IS_ERR(fp)) {
			found = 0;
			msleep(1800);
		} else {
...
{
	rmi4_tpd_hw_reset();
	if (fwu_need_force_upgrade())
		rmi4_tpd_reprobe(fwu->rmi4_data);
	rmi4_tpd_reinit(fwu->rmi4_data);
	syna_fwu_upgrade_progress = FWU_PROGRESS_PRE_DONE;
	fwu->rmi4_data->reset_device(fwu->rmi4_data);
	/* liuyw2 call scan pdt, some ic's bootloader had different f34 base addr
		as normal mode, call scan again to sync base addr
	*/
	fwu_scan_pdt();
}

static int fwu_kthread_hdlr(void *data)
{
	struct sched_param param = { .sched_priority = RTPM_PRIO_TPD };

	sched_setscheduler(current, SCHED_RR, &param);
	synaptics_fw_updater(data);
...
static void fwu_kthread_start(const void *data)
{
	...
	int retval;
	thread = kthread_run(fwu_kthread_hdlr, data, "tpd_fwu");
	if ( IS_ERR(thread) ) {
...
{
#if AUTO_UPDATE
	if (fwu_is_userspace_ctrl() || fwu_need_force_upgrade()) {
		...
		syna_fwu_upgrade_progress = FWU_PROGRESS_NOT_UPDATE;
#endif
}
...
{
	int retval;
	unsigned char attr_count, bit3203;
	...
	FAMILY_QUERY_ID = tpd_read_custom_family_query();
	__dbg("FW_FAMILY_QYERY_ID is 0x%02x.\n",FAMILY_QUERY_ID);

	bit3203 = (1 << FAMILY_QUERY_ID) & S3203_BIT;
	tpd_rmi4_s3203_det = bit3203 ? 1 : 0;

	/*lenovo-sw xuwen1 add 20140409 for read FW&ID end*/
#ifdef DO_STARTUP_FW_UPDATE
	__dbg("creat update thread\n");
	fwu->fwu_workqueue = create_singlethread_workqueue("fwu_workqueue");

	if (FAMILY_QUERY_ID_VENDOR1 && FAMILY_QUERY_ID == FAMILY_QUERY_ID_VENDOR1) {//vendor1
		cfwu_data = SynaFirmware_vendor1;
		curr_lastest_fwid = FW_ID_CURR_VENDOR1;
		INIT_DELAYED_WORK(&fwu->fwu_work, fwu_startup_fw_update_work);
		if (fwu_need_force_upgrade()) {
			queue_delayed_work(fwu->fwu_workqueue,
					&fwu->fwu_work, msecs_to_jiffies(1000));
		} else if(FW_ID_B_UPDAT < FW_ID_CURR_VENDOR1) {//IF you need to update FW ,please open this
			queue_delayed_work(fwu->fwu_workqueue,
					&fwu->fwu_work, msecs_to_jiffies(STARTUP_FW_UPDATE_DELAY_MS));
		} else
			syna_fwu_upgrade_progress = FWU_PROGRESS_NOT_UPDATE;
	}
	#ifdef SYNA_SUPPORT_VENDOR2
	else if (FAMILY_QUERY_ID_VENDOR2 && FAMILY_QUERY_ID == FAMILY_QUERY_ID_VENDOR2) {//vendor2
		cfwu_data = SynaFirmware_vendor2;
		curr_lastest_fwid = FW_ID_CURR_VENDOR2;
		INIT_DELAYED_WORK(&fwu->fwu_work, fwu_startup_fw_update_work);
		if (fwu_need_force_upgrade()) {
			queue_delayed_work(fwu->fwu_workqueue,
					&fwu->fwu_work, msecs_to_jiffies(1000));
		} else if(FW_ID_B_UPDAT < FW_ID_CURR_VENDOR2) {
			queue_delayed_work(fwu->fwu_workqueue,
					&fwu->fwu_work, msecs_to_jiffies(STARTUP_FW_UPDATE_DELAY_MS));
		} else
			syna_fwu_upgrade_progress = FWU_PROGRESS_NOT_UPDATE;
	...
	else if (FAMILY_QUERY_ID_VENDOR3 && FAMILY_QUERY_ID == FAMILY_QUERY_ID_VENDOR3) {//vendor2
		cfwu_data = SynaFirmware_vendor3;
		curr_lastest_fwid = FW_ID_CURR_VENDOR3;
		INIT_DELAYED_WORK(&fwu->fwu_work, fwu_startup_fw_update_work);
		if (fwu_need_force_upgrade()) {
			queue_delayed_work(fwu->fwu_workqueue,
					&fwu->fwu_work, msecs_to_jiffies(1000));
		} else if(FW_ID_B_UPDAT < FW_ID_CURR_VENDOR3) {
			queue_delayed_work(fwu->fwu_workqueue,
					&fwu->fwu_work, msecs_to_jiffies(STARTUP_FW_UPDATE_DELAY_MS));
		} else
			syna_fwu_upgrade_progress = FWU_PROGRESS_NOT_UPDATE;
	...
	else if (FAMILY_QUERY_ID_VENDOR4 && FAMILY_QUERY_ID == FAMILY_QUERY_ID_VENDOR4) {//vendor2
		cfwu_data = SynaFirmware_vendor4;
		curr_lastest_fwid = FW_ID_CURR_VENDOR4;
		INIT_DELAYED_WORK(&fwu->fwu_work, fwu_startup_fw_update_work);
		if (fwu_need_force_upgrade()) {
			queue_delayed_work(fwu->fwu_workqueue,
					&fwu->fwu_work, msecs_to_jiffies(1000));
		} else if(FW_ID_B_UPDAT < FW_ID_CURR_VENDOR4) {
			queue_delayed_work(fwu->fwu_workqueue,
					&fwu->fwu_work, msecs_to_jiffies(STARTUP_FW_UPDATE_DELAY_MS));
		} else
			syna_fwu_upgrade_progress = FWU_PROGRESS_NOT_UPDATE;
