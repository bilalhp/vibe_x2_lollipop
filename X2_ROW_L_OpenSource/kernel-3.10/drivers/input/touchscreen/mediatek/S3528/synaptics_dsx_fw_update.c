...
#include "SynaImage_015200a1.h"
/*lenovo-sw xuwen1 add 20140612 for two */
#include"SynaImage_04520006.h"
#include "tpd.h"
...
unsigned int tpd_read_fw_version(void)
{
	...
	if (retval < 0) {
		printk("[TSP-xw]Failed to read config (code %d).\n", retval);
		return retval;
	}
	printk("[TSP-xw]Device config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
...
tpd_info_t ->types = ((tpd_read_fw_version())>>6)&&0xFF;
...
{
	printk ("[TSP-xw]entern in update flow\n");//add by xuwen 1 for test
#ifdef FORCE_UPDATE
...
/*lenovo-sw xuwen1 add 20140613 for tp of boen begin*/
static void fwu_startup_fw_update_work_boen(struct work_struct *work)
{
       //printk ("[TSP-xw]entern in boen update flow\n");//add by xuwen 1 for test 
       #ifdef FORCE_UPDATE	
     //synaptics_fw_updater(SynaBoen);If you need to update Boen,please open this 	
    //#else 	
     synaptics_fw_updater(NULL);	
      #endif	
...
/*lenovo-sw xuwen1 add 20140613 for tp of boen end*/
...
/*lenovo-sw xuwen1 add 20140611 for version read begin*/
       unsigned int FW_ID_B_UPDAT;
       unsigned int FW_ID_A_UPDAT;
       unsigned int TP_FLAG;
/*lenovo-sw xuwen1 add 20140611 for version read end*/	   
...
printk("[TSP-xw]FW_ID_before_update is 0x%08x.\n",FW_ID_B_UPDAT);

/*lenovo-sw xuwen1 add 20140409 for read FW&ID end*/
TP_FLAG = FW_ID_B_UPDAT >> 0x18;
printk("[TSP-xw]TP_FLAG is 0x%08x.\n",TP_FLAG);
#ifdef DO_STARTUP_FW_UPDATE
if(TP_FLAG == 0x01) //tp of ofilm 
{
  if(FW_ID_B_UPDAT < 0x015200A1) //IF you need to update FW ,please open this 
{
...
else if(TP_FLAG == 0x04)//tp of boen 
   {
    if(FW_ID_B_UPDAT < 0x04520006) 
      { 
      fwu->fwu_workqueue = create_singlethread_workqueue("fwu_workqueue");
      INIT_DELAYED_WORK(&fwu->fwu_work, fwu_startup_fw_update_work_boen);
       queue_delayed_work(fwu->fwu_workqueue,
       &fwu->fwu_work,	
       msecs_to_jiffies(STARTUP_FW_UPDATE_DELAY_MS));
      } 
...
get_tpd_info();
//printk("[TSP-xw]FW_ID_after_update is 0x%08x.\n",FW_ID_A_UPDAT);
	/*lenovo-sw xuwen1 add 20140409 for read FW&ID end*/
	return 0;
