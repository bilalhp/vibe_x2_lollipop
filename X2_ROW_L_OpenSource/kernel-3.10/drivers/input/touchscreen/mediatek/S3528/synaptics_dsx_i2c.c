...
//#define LENOVO_CTP_SLIDE_WAKEUP //add by xuwen1 for gesture. //close gesture feature,mediatek
...
#ifdef KERNEL_ABOVE_2_6_38
//#define TYPE_B_PROTOCOL	//use the A protocol for mediatek factory test,mediatek
#endif
...
#define TPD_CLOSE_POWER_IN_SLEEP	//close tp in suspend,mediatek
#ifdef	TPD_HAVE_BUTTON
...
/*lenovo-sw xuwen1 add for gesture begin*/
      if ( rmi4_data->fn11_mask) {
		...
		if (lpwg_handler.lpwg_mode) {
			...
			if (retval < 0)
				return;
			printk("[TSP-xw]%s, lpwg_int_flag = %d.\n", __func__,lpwg_int_flag);
			printk("[TSP-xw]%s, gesture_type = 0x%x 0x%x 0x%x 0x%x 0x%x.\n",__func__,lpwg_handler.data_04.gesture_type,lpwg_handler.data_04.gesture_prop0,lpwg_handler.data_04.gesture_prop1,lpwg_handler.data_04.gesture_prop2,lpwg_handler.data_04.gesture_prop3);
			if (((lpwg_handler.data_04.gesture_type ==0x03)||(lpwg_handler.data_04.gesture_type == 0x07)
				||(lpwg_handler.data_04.gesture_type == 0x0A)||(lpwg_handler.data_04.gesture_type == 0x0B))
				&& (lpwg_int_flag == 1)) {
                  //if (lpwg_handler.data_04.gesture_type){
				  	printk("[TSP-xw]enter in gesture mode report KEY_POWER after\n");
				  if(lpwg_handler.data_04.gesture_type == 0x03)
				  	letter = 0x24;//double clic
				  else if(lpwg_handler.data_04.gesture_type == 0x07)
				  	letter = 0x20;//slide
				  else if(lpwg_handler.data_04.gesture_type == 0x0A)
				  	letter = 0x35;//v				 
				 else if (lpwg_handler.data_04.gesture_type == 0x0B)
				  	{
				if(lpwg_handler.data_04.gesture_prop1 == 0x63)
					letter = 0x34;//c
				else if(lpwg_handler.data_04.gesture_prop1 == 0x65)
					letter = 0x33;//e
				else if(lpwg_handler.data_04.gesture_prop1 == 0x6d)
					letter = 0x32;//m
				  	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		printk("tpd button, button=%d, status=%d, oldstatus=%d, touch_count_real=%d\n", button, status, current_status[button], touch_count_real);
#endif
	...
	if(touch_count && (0 == touch_count_real)) {
		printk("tpd button up, touch_count_real=%d\n", touch_count_real);
		input_report_key(rmi4_data->input_dev,
				BTN_TOUCH, 0);
		input_report_key(rmi4_data->input_dev,
				BTN_TOOL_FINGER, 0);
	...
	if (touch_count) {
		printk("tpd button input_sync, touch_count=%d\n", touch_count);
		input_sync(rmi4_data->input_dev);
	}
	mutex_unlock(&rmi4_report_mutex);
	return;
...
static void tpd_eint_handler(void)
{      
        printk("[TSP-xw]enter in tpd_eint_handler\n");
	TPD_DEBUG_PRINT_INT;
	tpd_flag=1;
...
/*#ifdef TPD_HAVE_BUTTON
	...
	dev_dbg(&rmi4_data->i2c_client->dev,
        ...
        printk("[TSP-xw1] max x = %d max y = %d\n ",rmi4_data->sensor_max_x,rmi4_data->sensor_max_y);
		...
		unsigned int intr_count)
{
	...
	ctrl_8_offset = query_5.ctrl0_is_present +
	 ...
	 printk("[TSP-xw]lpwg_handler.control_20.addr:0x%04x, lpwg_handler.data_04.addr:0x%04x, fhandler.ctrl_base:0x%04x,fhandler.data_base:0x%04x\n",lpwg_handler.control_20.addr,lpwg_handler.data_04.addr,fhandler->full_addr.ctrl_base,fhandler->full_addr.data_base);
         retval = synaptics_rmi4_i2c_read(rmi4_data,
	...
	if (retval < 0)
		return retval;
	printk("[TSP-xw] lpwg_handler.control_20.report_flags is %d.\n", lpwg_handler.control_20.report_flags);
...
/*#ifdef TPD_HAVE_BUTTON
	...
	dev_dbg(&rmi4_data->i2c_client->dev,
			...
			rmi4_data->sensor_max_x,
			rmi4_data->sensor_max_y);
      printk("[TSP-xw2] max x = %d max y = %d\n ",rmi4_data->sensor_max_x,rmi4_data->sensor_max_y);
...
#ifdef TYPE_B_PROTOCOL
	input_mt_init_slots(rmi4_data->input_dev,rmi4_data->num_of_fingers,2);//,0 lenovo-sw xuwen1 add for test
#endif
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	set_bit(BTN_TOOL_FINGER, rmi4_data->input_dev->keybit);
	 for(i=0;i<MAX_KEY_NUM;i++)
	   __set_bit(cap_button_codes[i], rmi4_data->input_dev->keybit);
...
static void synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
      ...
      printk("[TSP-xw]1/the status of sensor_sleep is %s\n",rmi4_data->sensor_sleep);
       #else
	device_ctrl = (device_ctrl & ~MASK_3BIT);
	...
	if (retval < 0) {
      ...
      printk("[TSP-xw]2/the status of sensor_sleep is %d \n",rmi4_data->sensor_sleep);
       #endif
	return;
}
...
if(get_tpd_suspend_status())
  {
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			lpwg_handler.control_20.data,
			sizeof(lpwg_handler.control_20.data));
	printk("[TSP-xw]:suspend_2/data is %d \n",lpwg_handler.control_20.report_flags);
...
{
		printk("[TSP-xw]%s, enter resume lpwg.\n", __func__);
	lpwg_handler.control_20.report_flags = ( lpwg_handler.control_20.report_flags)&0xFD;
	retval = synaptics_rmi4_i2c_write(rmi4_data,
			 lpwg_handler.control_20.addr,
			lpwg_handler.control_20.data,
			sizeof(lpwg_handler.control_20.data));
	printk("[TSP-xw] resume_1/lpwg_handler.control_20.report_flags is %d.\n", lpwg_handler.control_20.report_flags);
		if (retval < 0) {
		dev_err(&(rmi4_data->input_dev->dev),
				"%s: Failed to enter sleep mode\n",
				__func__);
		...
		printk("[TSP-xw]resume_2/ lpwg_handler.control_20.report_flags is %d.\n", lpwg_handler.control_20.report_flags);
		lpwg_handler.supported = true;
		lpwg_handler.lpwg_mode = false;
		lpwg_flag = 0;
		lpwg_int_flag = 0;
...
static struct tpd_driver_t synaptics_rmi4_driver = {
	...
	.suspend = synaptics_rmi4_suspend,
	.resume = synaptics_rmi4_resume,
#ifdef TPD_HAVE_BUTTON //lenovo-sw xuwen1 add for read version
	.tpd_have_button = 1,
#else
	.tpd_have_button = 0,
#endif		
