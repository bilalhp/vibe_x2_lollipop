...
/*Begin lenovo sw-chailu1 add for po charging 2014-06-10*/
#if defined(CONFIG_LENOVO_PO_CHARGING_SUPPORT)
static enum power_supply_property po_props[] = {
    POWER_SUPPLY_PROP_ONLINE,
};
#endif
/*End lenovo sw-chailu1 add for po charging 2014-06-10*/
...
/*Begin lenovo sw-chailu1 add for po charging 2014-06-10*/
#if defined(CONFIG_LENOVO_PO_CHARGING_SUPPORT)
static int po_get_property(struct power_supply *psy,
    enum power_supply_property psp,
    union power_supply_propval *val)
{
    int ret = 0;
    struct po_data *data = container_of(psy, struct po_data, psy);    

    switch (psp) {
    case POWER_SUPPLY_PROP_ONLINE:                           
        val->intval = data->PO_ONLINE;
        break;
    default:
        ret = -EINVAL;
        break;
...
/*Begin lenovo sw-chailu1 add for po charging 2014-06-10*/
#if defined(CONFIG_LENOVO_PO_CHARGING_SUPPORT)
/* po_data initialization */
static struct po_data po_main = {
    .psy = {
    .name = "po",
    .type = POWER_SUPPLY_TYPE_PO,
    .properties = po_props,
    .num_properties = ARRAY_SIZE(po_props),
    .get_property = po_get_property,                
    },
    .PO_ONLINE = 0,
};
#endif
...
/*Begin lenovo sw-chailu1 add for po charging FMT 2014-06-14*/
#if defined(CONFIG_LENOVO_PO_CHARGING_SUPPORT)
static ssize_t show_po_fmt(struct device *dev,struct device_attribute *attr, char *buf)
{
    battery_xlog_printk(BAT_LOG_CRTI, "po_fmt = %d\n",is_po_ftm);    
    return sprintf(buf, "%u\n", is_po_ftm);
}
static ssize_t store_po_fmt(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	sscanf(buf, "%u", &is_po_ftm);
    battery_xlog_printk(BAT_LOG_CRTI, "po_fmt= %d\n",is_po_ftm); 
	wake_up_bat ();
	
    return size;
}
static DEVICE_ATTR(po_fmt, 0664, show_po_fmt, store_po_fmt);
#endif
/*Begin lenovo sw-chailu1 add for po charging FMT 2014-06-14*/
...
/*Begin lenovo sw-chailu1 add for po charging 2014-06-10*/
#if defined(CONFIG_LENOVO_PO_CHARGING_SUPPORT)
static void po_update(struct po_data *po_data)
{
    struct power_supply *po_psy = &po_data->psy;

    if( BMT_status.charger_exist == KAL_TRUE )
    {         
        if (BMT_status.charger_type == PO_CHARGER)
             
        {
			printk("po_update po online = 1 \n");
            po_data->PO_ONLINE = 1;        
            po_psy->type = POWER_SUPPLY_TYPE_PO;
        }
        else
        {
           po_data->PO_ONLINE = 0; 
					   ...
					   kal_int32 *sum)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		...
		/*Lenovo-sw begin chailu1 add 2013-6-4,add for lenovo charging*/ 
        #if defined(CONFIG_LENOVO_PO_CHARGING_SUPPORT)
            else if (BMT_status.charger_type == PO_CHARGER)
				data = PO_CHARGER_CURRENT / 100;		//mA
        #endif
        /*Lenovo-sw end chailu1 add 2013-6-4,add for lenovo charging*/ 		
			else	/* USB */
...
/*End,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
	...
	if (bat_meter_timeout == KAL_TRUE || bat_spm_timeout == TRUE) {
		...
		/*Begin lenovo-sw chailu1 add for TI's FG 20140909*/
        ...
        /*End lenovo-sw chailu1 add for TI's FG 20140909*/
...
/*Begin,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/	
#if defined(SOC_BY_3RD_FG) 	
    if((BMT_status.SOC <= 1) && (BMT_status.bat_vol > 3900) && (bat_vol > 3900) && (BMT_status.charger_exist == KAL_FALSE)   && (g_bat_init_flag == KAL_TRUE))
    {
          BMT_status.SOC = 46;
	}
#endif
...
CHARGER_TYPE mt_charger_type_detection(void)
{
	...
	if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
		...
		{
			battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE,&CHR_Type_num);
			printk("mt_charger_type_detection:  second check  chr typ = %d \n",CHR_Type_num);
		}
		BMT_status.charger_type = CHR_Type_num;	
#else			
        BMT_status.charger_type = CHR_Type_num;	
...
static void mt_kpoc_power_off_check(void)
{
	...
	if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
	    ...
	    /*Begin,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
             #ifdef CONFIG_LENOVO_CHARGING_STANDARD_SUPPORT
		   if ((upmu_is_chr_det() == KAL_FALSE) && (BMT_status.charger_vol < 2500)&& (g_bat_init_flag == KAL_TRUE))
	      #else	 
...
/*Begin,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
        #ifdef CONFIG_LENOVO_CHARGING_STANDARD_SUPPORT	
		    } else if ( charging_level_data[0] == 16 ) {    charging_level_data[0] = CHARGE_CURRENT_0_00_MA;
		#endif	
/*End,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
		 }else {
			charging_level_data[0] = CHARGE_CURRENT_450_00_MA;
...
/*Begin lenovo sw-chailu1 add for po charging 2014-06-10*/
#if defined(CONFIG_LENOVO_PO_CHARGING_SUPPORT)
    ret = power_supply_register(&(dev->dev), &po_main.psy);
    if (ret)
    {            
        battery_xlog_printk(BAT_LOG_CRTI, "[BAT_probe] power_supply_register PO Fail !!\n");                    
        return ret;
    }             
    battery_xlog_printk(BAT_LOG_CRTI, "[BAT_probe] power_supply_register PO Success !!\n");
#endif
/*End lenovo sw-chailu1 add for po charging 2014-06-10*/
...
/*End,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
	...
	/* For EM */
	{
		...
		/*Begin lenovo sw-chailu1 add for po charging FMT 2014-06-14*/
        #if defined(CONFIG_LENOVO_PO_CHARGING_SUPPORT)
        ret_device_file = device_create_file(&(dev->dev), &dev_attr_po_fmt);
		#endif
	    /*Begin lenovo sw-chailu1 add for po charging FMT 2014-06-14*/
	}
