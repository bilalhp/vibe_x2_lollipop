...
/*Begin,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
#ifdef CONFIG_LENOVO_CHARGING_STANDARD_SUPPORT
#include "lenovo_charging_3rd_fg.h"
#endif
/*End,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
/* ============================================================ // */
/* define */
/* ============================================================ // */
...
#endif
//XTWOROWL-310 begin
//U32 _g_bat_wakeup_total_time = 0;
//XTWOROWL-310 end
...
//XTOWROWL-310
//struct timespec g_rtc_time_after_sleep;
//struct timeval g_rtc_time_after_sleep_tv;
//struct timeval g_rtc_time_before_sleep_tv;
int g_bat_capacity_ic = 50;
void fgauge_construct_battery_profile(kal_int32 temperature, BATTERY_PROFILE_STRUC_P temp_profile_p);
extern short bq27531_set_reset(void);
u64 g_battery_wakeup_pre_ms = 0;
u64 g_bat_wakeup_total_s = 0 ;
//XTOWROWL-310
...
/*Begin,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
#ifdef CONFIG_LENOVO_CHARGING_STANDARD_SUPPORT
static int g_is_bat_temp_pin = 1; 
int force_get_tbat();
int is_batt_temp_pin(void)	
{	
	force_get_tbat();
    return 	g_is_bat_temp_pin;
}
#endif
...
kal_int32 fgauge_read_v_by_capacity_bq27531(int bat_capacity,kal_int32 temperature)
{    
    int i = 0, saddles = 0;
    BATTERY_PROFILE_STRUC_P profile_p;
    kal_int32 ret_volt = 0;
//    kal_int32 temperature = 25;
	g_bat_capacity_ic = 100 - bat_capacity ;	
	profile_p = fgauge_get_profile(TEMPERATURE_T);
//    temperature = ;	
    fgauge_construct_battery_profile(temperature,profile_p);//fgauge_get_profile(TEMPERATURE_T);
//	fgauge_get_profile(TEMPERATURE_T);
    ...
    printk(KERN_ERR " %s ,ret_volt:%d",__func__,ret_volt);
    return ret_volt;
}
//XTWOROWL-310
...
/*Begin,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
    ...
    if(vbat_val > 3700)				//3.7v
        tmp =NORMAL_WAKEUP_PERIOD; // 
    else if(vbat_val > VBAT_NORMAL_WAKEUP)     //3.6v 
        tmp = 600; // 10 min
    else if(vbat_val > VBAT_LOW_POWER_WAKEUP)     //3.5v   
        tmp = LOW_POWER_WAKEUP_PERIOD; // 5 min
    else
        tmp = CLOSE_POWEROFF_WAKEUP_PERIOD; // 0.5 min

		

/*Begin lenovo-sw chailu1 add for chaging sleep 2014-07-15*/
#ifdef LENOVO_CHARGING_SLEEP_ENABLE
...
#endif
/*End lenovo-sw chailu1 add for chaging sleep 2014-07-15*/
        if(tmp > LENOVO_3RD_FG_SPM_TMR)
              g_spm_timer = LENOVO_3RD_FG_SPM_TMR;
	 else
	 	g_spm_timer = tmp;
	bm_print(BM_LOG_CRTI, "3rd_fg: vbat_val=%d, pcm timer=%d,g_spm_timer=%d \n", vbat_val, tmp,g_spm_timer);
...
/*Begin,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
#ifdef CONFIG_LENOVO_CHARGING_STANDARD_SUPPORT
static ssize_t show_is_Battery_exist(struct device *dev,struct device_attribute *attr, char *buf)
{    
	force_get_tbat(KAL_TRUE);
    bm_print(BM_LOG_CRTI, "show_is_Batter_exist  : %d\n", g_is_bat_temp_pin );	

    return sprintf(buf, "%d\n", g_is_bat_temp_pin );
}
static ssize_t store_is_Battery_exist(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{      
    return size;
}
static DEVICE_ATTR(is_Battery_exist, 0664, show_is_Battery_exist, store_is_Battery_exist);
...
#endif
//XTWOROWL-310
//do_gettimeofday(&g_rtc_time_after_sleep_tv);
g_battery_wakeup_pre_ms = ktime_to_ms(ktime_get());
printk(KERN_ERR "gauge reset g_battery_wakeup_pre_ms:%lld",g_battery_wakeup_pre_ms);
//XTWOROWL-310

/*Begin,Lenovo-sw chailu1 modify 2015-2-5, support TI's FG fuction*/
#ifdef CONFIG_LENOVO_CHARGING_STANDARD_SUPPORT
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_is_Battery_exist);
#endif
...
//XTWOROWL-310
	static int soc = 51;
	static int pre_soc = 49;
	static int counter = 0;
        u64 bat_wakeup_curr_ms = ktime_to_ms(ktime_get());
//XTWOROWL-310
#if defined(CONFIG_POWER_EXT)
...
#elif defined(SOC_BY_SW_FG) || defined(SOC_BY_HW_FG) || defined(SOC_BY_3RD_FG) 
{
		...
		//lenovo-sw mahj2 modify for update sleep time at 20150302 End
//XTWOROWL-310
	g_bat_wakeup_total_s += (bat_wakeup_curr_ms - g_battery_wakeup_pre_ms );
	printk(KERN_ERR " gauge reset battery_meter_suspend, wakeup_total_time_s:%lld,soc:%d,pre_soc:%d,g_battery_wakeup_pre_ms:%lld,bat_wakeup_curr_ms:%lld \n",g_bat_wakeup_total_s,g_bat_capacity_ic,pre_soc,g_battery_wakeup_pre_ms,bat_wakeup_curr_ms);

	soc = g_bat_capacity_ic ;
	g_battery_wakeup_pre_ms = bat_wakeup_curr_ms ;
	if(pre_soc != soc )
	{
		g_bat_wakeup_total_s = 0;
		pre_soc = soc ;
	}
	
	if( g_bat_wakeup_total_s >= 7200000 )
	{
		g_bat_wakeup_total_s = 0;
		if(pre_soc == soc)
		{
			printk(KERN_ERR "two hour preoc == soc,trigger gauge reset");
			bq27531_set_reset();
		}
	}
//XTWOROWL-310
...
//XTWOROWL-310
	//do_gettimeofday(&g_rtc_time_after_sleep_tv);
//	printk(KERN_ERR "gauge reset ktime:%lld",curr);
//XTWOROWL-310
...
#if defined(SOC_BY_HW_FG) || defined(SOC_BY_3RD_FG) 
#ifdef MTK_ENABLE_AGING_ALGORITHM
    if(bat_is_charger_exist() == KAL_FALSE)
    {
        battery_aging_check();
